...
#   OpenClaw），读取当前目录或仓库根的 AutoTestAndSaveReport.md 作为提示词
#   执行自动化测试；Agent 退出后自动将 TestReport 中的报告文件以中文 git
#   提交（子模块 UseReport 在子仓库内单独提交），随后由 shell 层确定性接力
#   启动 AutoDebugTestReport.sh 进入自动修复流程，全程 bypass 权限。
#
# 特性：
...
# ---------- 随机选择 Agent ----------
pick_agent "AutoTestAndSaveReport"
...

{
    ...
    echo "[AutoTestAndSaveReport] 选中 Agent : ${SELECTED_AGENT}"
    echo "[AutoTestAndSaveReport] Agent 二进制: ${AGENT_BIN_PATH}"
    echo "============================================================"
} >> "${LOG_FILE}"
...
nohup setsid bash -c "
    ...
    AGENT_EXIT=\$?
    echo '[AutoTestAndSaveReport] ${SELECTED_AGENT} 退出码 : '\${AGENT_EXIT}
    echo '[AutoTestAndSaveReport] ${SELECTED_AGENT} 结束时间 : '\"\$(date '+%F %T')\"
...
echo "[AutoTestAndSaveReport] 已后台启动 Agent [${SELECTED_AGENT}]，PID=${AGENT_PID}"
echo "[AutoTestAndSaveReport] 日志 : ${LOG_FILE}"
echo "[AutoTestAndSaveReport] Agent 退出后会自动 git add + git commit（中文提交信息），"
