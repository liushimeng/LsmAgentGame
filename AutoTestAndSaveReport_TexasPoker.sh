...
#   - 日志输出到 ./logs/auto_test_texasholdem_<timestamp>.log(§20260820-01
#     由原 `auto_test_texas_*.log` 补齐为德扑完整拼音)
...
LOG_FILE="${LOG_DIR}/auto_test_texasholdem_${TS}.log"
...
# ---------- 启动日志头 ----------
print_section_header "AutoTestAndSaveReport_TexasPoker" "${PROMPT_FILE}" "${LOG_FILE}" "${PROJECT_DIR}" "${SELECTED_AGENT}"
...
start_agent_in_background "${LOG_FILE}" "
    ...
    git_add_safe \"TestReport/\${TEXAS_MAIN_GLOB}\" || echo '[AutoTestAndSaveReport_TexasPoker] 警告: TestReport/德扑主报告无可暂存内容(已忽略)'
    ...
    if git diff --cached --quiet; then
        ...
        git_commit_chinese '测试' 'texasholdem' \"\${COMMIT_TS}\" 'TestReport/德扑报告 + go-web-debug-tool 子模块 gitlink(如有)' \\
...
echo "[AutoTestAndSaveReport_TexasPoker] 已后台启动 Agent [${SELECTED_AGENT}]"
