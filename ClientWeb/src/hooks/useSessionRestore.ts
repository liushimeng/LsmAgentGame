...
// 支持：/xiangqi/:roomId、/chess/:roomId、/junqi/:roomId、/doudizhu/:roomId、/texasholdem/:roomId、/virtual-city/:roomId
// 以及 spectator 兄弟路由：/<game>/spectate/:roomId。
function parseGameRoute(pathname: string):
  | { gameKind: 'xiangqi' | 'chess' | 'junqi' | 'doudizhu' | 'texasholdem' | 'werewolf' | 'virtual_city'; roomId: string; joinExtra: Record<string, unknown>; spectator: boolean }
  | null {
  const m = pathname.match(/^\/(xiangqi|chess|junqi|doudizhu|texasholdem|werewolf|virtualCity)(?:\/spectate)?\/([^/]+)$/);
  if (!m) return null;
  const gameKind = m[1] as 'xiangqi' | 'chess' | 'junqi' | 'doudizhu' | 'texasholdem' | 'werewolf' | 'virtual_city';
