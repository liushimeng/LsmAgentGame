...
import type { WealthSociety, WealthMyState } from '@/types/wealth';

interface Props {
  society: WealthSociety | null | undefined;
  my: WealthMyState | null | undefined;
}
...
export function LorenzCurvePanel({ society, my }: Props) {
  ...
  if (!society || !society.lorenz_points || society.lorenz_points.length < 2) {
    return (
      <div className="wealth-lorenz">
        <div className="wealth-lorenz__title">
          <span>{t('wealth.dashboard.lorenz.title' as TKey)}</span>
        </div>
        <div className="wealth-lorenz__empty">{t('wealth.dashboard.lorenz.empty' as TKey)}</div>
      </div>
    );
  }

  const pts = society.lorenz_points; // [(popFrac, wealthFrac)]
  ...
  return (
    <div className="wealth-lorenz">
      <div className="wealth-lorenz__title">
        <span>{t('wealth.dashboard.lorenz.title' as TKey)}</span>
        <span className="wealth-lorenz__gini">
          {t('wealth.dashboard.lorenz.giniBadge' as TKey, {
      ...
      <svg
        ...
        role="img"
        aria-label={t('wealth.dashboard.lorenz.title' as TKey)}
      >
        {/* 坐标轴 */}
        <line
          x1={PAD_X}
        ...
        {myNet > 0 && society.total_wealth && society.total_wealth > 0 && rankDesc && (
          <MyPositionDot
            ...
            label={t('wealth.dashboard.lorenz.myPos' as TKey)}
          />
        )}
      </svg>
      </div>
      <div className="wealth-lorenz__legend">
        <span>
          ...
          {t('wealth.dashboard.lorenz.equalLine' as TKey)}
        </span>
        <span>
          <i className="wealth-lorenz__legend-dot wealth-lorenz__legend-dot--curve" />
          {t('wealth.dashboard.lorenz.lorenzCurve' as TKey)}
        </span>
        {myNet > 0 && (
          <span style={{ color: '#f472b6' }}>
            {t('wealth.dashboard.lorenz.myPos' as TKey)}: ¥{formatCny(myNet)}
          </span>
        )}
      </div>
    </div>
...
function MyPositionDot({
  ...
  return (
    <g>
      <circle cx={x} cy={y} r={5} className="wealth-lorenz__me" />
      <text x={x + 8} y={y - 6} className="wealth-lorenz__me-label">
        {label}
      </text>
    </g>
  );
