...
export function CivicElectionBanner({ election, month, mayorNickname }: Props) {
  ...
  const termElapsed = wealthElectionTermElapsed(month, next);
  ...
  return (
    <div className="wealth-election-banner" role="status" data-testid="wealth-election-banner">
      <span className="wealth-election-banner__title">🗳 {t('wealth.election.title' as TKey)}</span>
      <span data-testid="wealth-election-mayor">
        {t('wealth.election.mayor' as TKey, {
          seat: mayorSeat + 1,
          name: mayorNickname ? ` · ${mayorNickname}` : '',
        })}
      </span>
      <span className="wealth-election-banner__term" title={t('wealth.election.termProgress' as TKey, { elapsed: termElapsed, interval: WEALTH_ELECTION_INTERVAL_MONTHS })}>
        <span
          className="wealth-election-banner__track"
          role="progressbar"
          aria-valuenow={termPct}
          aria-valuemin={0}
          aria-valuemax={100}
        >
          <span className="wealth-election-banner__fill" style={{ width: `${termPct}%` }} aria-hidden="true" />
        </span>
        {termPct}%
      </span>
      {next > 0 && (
        <span>{t('wealth.election.nextElection' as TKey, { m: next })}</span>
      )}
      {stipendStopped && (
        <span className="wealth-election-banner__stipend" data-testid="wealth-election-stipend">
          ⚠ {t('wealth.election.stipendStopped' as TKey)}
        </span>
      )}
      <button
        type="button"
        className="wealth-election-banner__close"
        onClick={() => setDismissedKey(bannerKey)}
        aria-label={t('wealth.election.bannerClose' as TKey)}
        data-testid="wealth-election-banner-close"
      >
        ✕ {t('wealth.election.bannerClose' as TKey)}
      </button>
    </div>
  );
}
