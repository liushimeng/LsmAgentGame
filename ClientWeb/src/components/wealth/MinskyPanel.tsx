...
/** 明斯基等级 i18n key。 */
function tierLabel(tier: WealthMinskyTier): TKey {
  return `minsky.${tier}` as TKey;
}

/** 明斯基等级徽章（颜色编码 + 白字对比度 ≥5:1）。 */
function MinskyBadge({ tier, count }: { tier: WealthMinskyTier; count: number }) {
  const t = useT();
  const color = minskyTierColor(tier);
  return (
    <span
      className="wealth-badge wealth-minsky-badge"
...
function PonziMeter({ ratio }: { ratio: number }) {
  ...
  return (
    <div className="wealth-minsky__meter" aria-label={t('minsky.ponziRatio' as TKey)}>
      <div className="wealth-minsky__meter-head">
        <span className="wealth-minsky__meter-label">
          {t('minsky.ponziRatio' as TKey)}
        </span>
        <span
          className={danger ? 'wealth-minsky__meter-value wealth-num--neg' : 'wealth-minsky__meter-value'}
        >
          {pct.toFixed(1)}%
        </span>
      </div>
      <div className="wealth-minsky__meter-track">
        <div
          className={danger ? 'wealth-minsky__meter-fill wealth-minsky__meter-fill--danger' : 'wealth-minsky__meter-fill'}
          style={{ width: `${pct}%` }}
        />
        {/* 30% 阈值标记线 */}
        <span className="wealth-minsky__meter-threshold" title={t('minsky.threshold' as TKey)} />
      </div>
      <div className="wealth-minsky__meter-foot">
        <span>0%</span>
        <span className="wealth-minsky__meter-threshold-label">
...
export function MinskyPanel({ gameState }: Props) {
  ...
  if (!overview) {
    return (
      <div className="wealth-minsky">
        <div className="wealth-minsky__head">
          <span className="wealth-minsky__title">{t('minsky.title' as TKey)}</span>
        </div>
    ...
    <div className="wealth-minsky">
      <div className="wealth-minsky__head">
        ...
        {(cycle?.minsky_moment_count ?? 0) > 0 && (
          <span className="wealth-badge wealth-minsky-badge--count">
      ...
      {/* 三档融资等级分布条 */}
      <div className="wealth-minsky__tiers">
      ...
      {/* 堆叠分布条（对冲/投机/庞氏） */}
      <div
        className="wealth-minsky__stack"
      ...
      >
        {hedgePct > 0 && (
          <span
            className="wealth-minsky__stack-fill wealth-minsky__stack-fill--hedge"
            style={{ width: `${hedgePct}%` }}
          />
        )}
        {specPct > 0 && (
          <span
            className="wealth-minsky__stack-fill wealth-minsky__stack-fill--spec"
            style={{ width: `${specPct}%` }}
          />
        )}
        {ponziPct > 0 && (
          <span
            className="wealth-minsky__stack-fill wealth-minsky__stack-fill--ponzi"
      ...
      {overview.cooldown_left > 0 && (
        <div className="wealth-minsky__cooldown">
          <span className="wealth-minsky__cooldown-icon" aria-hidden="true">🛡️</span>
          <span>
      ...
      {/* 存活人数备注 */}
      <div className="wealth-minsky__total">
        {t('minsky.totalAlive' as TKey, { n: totalAlive })}
      </div>

      {/* 教育向提示 */}
      <details className="wealth-minsky__edu">
        <summary>{t('minsky.eduToggle' as TKey)}</summary>
        <div className="wealth-minsky__edu-body">
          <p>{t('minsky.eduP1' as TKey)}</p>
          <p>{t('minsky.eduP2' as TKey)}</p>
          <p className="wealth-minsky__edu-warn">{t('minsky.eduWarn' as TKey)}</p>
        </div>
      </details>
    </div>
  );
