...
export function MinskyStatusBar({ gameState, mySeat }: Props) {
  ...
  const tier: VirtualCityMinskyTier | undefined = me?.minsky_tier;
  ...
  return (
    <div className={'virtualCity-minsky-statusbar' + (flashing ? ' virtualCity-minsky-moment-flash' : '')}>
      {/* 我的融资等级 */}
      {tier && (
        <span
          className="virtualCity-badge virtualCity-minsky-statusbar__badge"
      ...
      {overview && dangerRatio && (
        <span className="virtualCity-minsky-statusbar__warn">
      ...
      {overview && overview.cooldown_left > 0 && (
        <span className="virtualCity-minsky-statusbar__cooldown">
      ...
      {/* 时刻闪烁动画说明文字 */}
      {flashing && (
        <span className="virtualCity-minsky-statusbar__flash-text">
