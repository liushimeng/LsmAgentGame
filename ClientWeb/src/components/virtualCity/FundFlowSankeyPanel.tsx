 ...
 * - 空态:本月无流水 → 渲染 virtualCity-flow__empty。
 */

import { useT } from '@/hooks/useT';
import type { TKey } from '@/i18n';
import { formatCny } from '@/types/virtualCity';
import type { VirtualCityFlowStat, VirtualCityFlowNode, VirtualCityFlowLink } from '@/types/virtualCity';

interface Props {
  flowStat: VirtualCityFlowStat | null | undefined;
}
...
const NODE_LABEL_KEYS: Record<string, string> = {
  salary: 'virtualCity.dashboard.flow.node.salary',
  firms: 'virtualCity.dashboard.flow.node.firms',
  market: 'virtualCity.dashboard.flow.node.market',
  bank: 'virtualCity.dashboard.flow.node.bank',
  gov: 'virtualCity.dashboard.flow.node.gov',
  player: 'virtualCity.dashboard.flow.node.player',
  world: 'virtualCity.dashboard.flow.node.world',
};

const STROKE_CLASS: Record<string, string> = {
  source: 'virtualCity-flow__link--source',
  sink: 'virtualCity-flow__link--sink',
  pass: 'virtualCity-flow__link--pass',
};
...
export function FundFlowSankeyPanel({ flowStat }: Props) {
  ...
  if (!flowStat || !flowStat.links || flowStat.links.length === 0) {
    return (
      <div className="virtualCity-flow">
        <div className="virtualCity-flow__title">
          <span>{t('virtualCity.dashboard.flow.title' as TKey)}</span>
        </div>
        <div className="virtualCity-flow__empty">{t('virtualCity.dashboard.flow.empty' as TKey)}</div>
      </div>
    );
  }
  ...
  const outgoingByFrom = new Map<string, VirtualCityFlowLink[]>();
  for (const l of flowStat.links) {
    const arr = outgoingByFrom.get(l.from) || [];
    arr.push(l);
    outgoingByFrom.set(l.from, arr);
  }
  const incomingByTo = new Map<string, VirtualCityFlowLink[]>();
  for (const l of flowStat.links) {
  ...
  return (
    <div className="virtualCity-flow">
      <div className="virtualCity-flow__title">
        <span>{t('virtualCity.dashboard.flow.title' as TKey)}</span>
        <span className="virtualCity-flow__totals">
          <span className="virtualCity-flow__total--in">
            ↑ {t('virtualCity.dashboard.flow.in' as TKey)}: ¥{formatCny(flowStat.total_in_cny)}
          </span>
          <span className="virtualCity-flow__total--out">
            ↓ {t('virtualCity.dashboard.flow.out' as TKey)}: ¥{formatCny(flowStat.total_out_cny)}
          </span>
        </span>
      </div>
      ...
      <svg
        className="virtualCity-flow__svg"
        viewBox={`0 0 ${W} ${H}`}
        preserveAspectRatio="xMidYMid meet"
        role="img"
        aria-label={t('virtualCity.dashboard.flow.title' as TKey)}
      >
        {/* 边 */}
        {edges.map((e, i) => {
          ...
          const label = t('virtualCity.dashboard.flow.tooltip' as TKey, {
          ...
          return (
            <path
              key={`edge-${i}`}
              d={path}
              className={`virtualCity-flow__link ${kindClass}`}
...
function linkKind(link: VirtualCityFlowLink, nodes: VirtualCityFlowNode[]): string {
...
function NodeBox({
  ...
  return (
    <g>
      ...
      <text x={labelX} y={y + h / 2 - 2} className="virtualCity-flow__node-label" textAnchor={textAnchor}>
        {label}
      </text>
      <text x={amountX} y={y + h / 2 + 10} className="virtualCity-flow__node-amount" textAnchor={textAnchor}>
        ¥{formatCny(amount)}
      </text>
    </g>
  );
