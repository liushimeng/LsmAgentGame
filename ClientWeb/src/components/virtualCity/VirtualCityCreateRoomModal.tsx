    ...
    >
      <div className="virtualCity-create-form">
        ...
        <div className="virtualCity-create-form__row">
          <span>{t('virtualCity.residentCount' as TKey)}</span>
          <div className="virtualCity-create-form__seatcount" data-testid="virtualCity-create-resident-tiers">
            {RESIDENT_PRESETS.map((p) => (
              <button
                key={p.value}
                type="button"
                className={
                  'virtualCity-tier-btn virtualCity-tier-btn--sm' +
                  (residentCount === p.value ? ' virtualCity-tier-btn--active' : '')
                }
                onClick={() => setResidentCount(clampResidents(p.value))}
                disabled={busy}
                aria-pressed={residentCount === p.value}
                data-testid={`virtualCity-create-resident-tier-${p.value}`}
              >
                {p.label}
              </button>
            ))}
        ...
        <label className="virtualCity-create-form__row">
          <span>
            {t('virtualCity.residentCount' as TKey)} · {residentCount.toLocaleString()}
          </span>
          <input
            type="number"
            min={RESIDENT_MIN}
            ...
            disabled={busy}
            data-testid="virtualCity-create-resident-count"
          />
        </label>
