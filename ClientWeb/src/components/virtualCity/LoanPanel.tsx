...
import {
  ...
  type VirtualCityLoanDirection,
  type VirtualCityMyState,
  type VirtualCityP2PLoan,
...
export function LoanPanel({ gameState, mySeat, sendTrade, onRefresh }: Props) {
  ...
  // 发布借贷表单字段。
  const [direction, setDirection] = useState<VirtualCityLoanDirection>('lend');
  ...
  const handleCreate = () => {
    ...
    if (p < 1000) {
      alert(t('virtualCity.action.minAmount' as TKey, { n: 1000 }));
      return;
    }
    if (r < 0.3 || r > 3.6) {
      alert(t('virtualCity.error.loanRate' as TKey));
      return;
    }
  ...
  const handleAccept = (l: VirtualCityListing) => {
    if (isMine(l)) return;
    sendTrade({ type: 'loan_accept', listing_id: l.id });
  };

  const handleRepay = (loan: VirtualCityP2PLoan, full: boolean) => {
  ...
  const handleGuarant = (loan: VirtualCityP2PLoan) => {
  ...
  return (
    <div className="virtualCity-loanpanel">
      <div className="virtualCity-loanpanel__head">
        <span className="virtualCity-loanpanel__title">{t('virtualCity.loan.title' as TKey)}</span>
      ...
      <p className="virtualCity-loanpanel__subtitle">{t('virtualCity.loan.subtitle' as TKey)}</p>

      <div className="virtualCity-tabs">
        <button
          type="button"
          className={'virtualCity-tabs__btn' + (tab === 'market' ? ' virtualCity-tabs__btn--active' : '')}
          onClick={() => setTab('market')}
        >
          {t('virtualCity.loan.market' as TKey)}
        </button>
        <button
          type="button"
          className={'virtualCity-tabs__btn' + (tab === 'mine' ? ' virtualCity-tabs__btn--active' : '')}
          onClick={() => setTab('mine')}
        >
          {t('virtualCity.loan.mine' as TKey)}
        </button>
      </div>
      ...
      {tab === 'market' && (
        <>
          <table className="virtualCity-table">
            <thead>
              <tr>
                <th>{t('virtualCity.loan.col.id' as TKey)}</th>
                <th>{t('virtualCity.loan.col.direction' as TKey)}</th>
                <th>{t('virtualCity.loan.col.seat' as TKey)}</th>
                <th>{t('virtualCity.loan.col.principal' as TKey)}</th>
                <th>{t('virtualCity.loan.col.rate' as TKey)}</th>
                <th>{t('virtualCity.loan.col.term' as TKey)}</th>
                <th>{t('virtualCity.loan.col.actions' as TKey)}</th>
              </tr>
            </thead>
            <tbody>
              {marketListings
                ...
                .map((l) => {
                  ...
                  return (
                    <tr key={l.id} className={isMine(l) ? 'virtualCity-table__row--mine' : ''}>
                      ...
                      <td>
                        <span className={'virtualCity-badge virtualCity-badge--dir-' + pl.direction}>
                          {t(`virtualCity.loan.${pl.direction}` as TKey)}
                        </span>
                      </td>
                      ...
                      <td>
                        {isMine(l) ? (
                          ...
                          <button type="button" className="btn btn-mini btn-primary" onClick={() => handleAccept(l)}>
                            {t('virtualCity.loan.acceptBtn' as TKey)}
                          </button>
                        )}
                      </td>
                    </tr>
              ...
              {marketListings.filter((l) => l.status === 'open').length === 0 && (
                <tr><td colSpan={7} className="virtualCity-table__empty">{t('virtualCity.loan.empty' as TKey)}</td></tr>
              )}
            </tbody>
          </table>

          {/* 发布借贷表单 */}
          <div className="virtualCity-loanpanel__create">
            <div className="virtualCity-loanpanel__create-title">{t('virtualCity.loan.createTitle' as TKey)}</div>
            <div className="virtualCity-loanpanel__form">
              <label className="virtualCity-action-form__row">
                <span>{t('virtualCity.loan.selDirection' as TKey)}</span>
                <select value={direction} onChange={(e) => setDirection(e.target.value as VirtualCityLoanDirection)}>
                  <option value="lend">{t('virtualCity.loan.lend' as TKey)}</option>
                  <option value="borrow">{t('virtualCity.loan.borrow' as TKey)}</option>
                </select>
              </label>
              <label className="virtualCity-action-form__row">
                <span>{t('virtualCity.loan.principal' as TKey)}（≥1000）</span>
                <input type="number" min={1000} step={1000} value={principal} onChange={(e) => setPrincipal(e.target.value)} />
              </label>
              <label className="virtualCity-action-form__row">
                <span>{t('virtualCity.loan.maxRate' as TKey)}（0.3–3.6%）</span>
                <input type="number" min={0.3} max={3.6} step={0.1} value={maxRate} onChange={(e) => setMaxRate(e.target.value)} />
              </label>
              <label className="virtualCity-action-form__row">
                <span>{t('virtualCity.loan.termN' as TKey)}（1–60）</span>
              ...
              {direction === 'borrow' && (
                <label className="virtualCity-action-form__check">
                  ...
                  {t('virtualCity.loan.needGuarantee' as TKey)}
                </label>
              )}
              <button type="button" className="btn btn-primary" onClick={handleCreate}>
                {t('virtualCity.listing.confirmCreate' as TKey)}
              </button>
            </div>
          </div>
        </>
      ...
      {tab === 'mine' && (
        <>
          {myBorrowed.length > 0 && (
            <div className="virtualCity-loanpanel__section-title">{t('virtualCity.loan.borrow' as TKey)}</div>
          )}
          <table className="virtualCity-table">
            <thead>
              <tr>
                ...
                <th>{t('virtualCity.loan.col.principal' as TKey)}</th>
                <th>{t('virtualCity.loan.col.rate' as TKey)}</th>
                <th>{t('virtualCity.loan.col.balance' as TKey)}</th>
                <th>{t('virtualCity.loan.col.monthsLeft' as TKey)}</th>
                <th>{t('virtualCity.loan.col.guarantor' as TKey)}</th>
                <th>{t('virtualCity.loan.col.actions' as TKey)}</th>
              </tr>
            </thead>
            <tbody>
              {myBorrowed.map((l) => (
                <tr key={l.id} className={l.overdue ? 'virtualCity-table__row--overdue' : ''}>
                  <td className="virtualCity-table__id">{l.id}</td>
                  <td>¥{formatCny(l.principal_cny)}</td>
                  <td>{formatPct(l.annual_rate, 1)}</td>
                  <td className="virtualCity-num--neg">¥{formatCny(l.balance_cny)}</td>
                  ...
                  <td>
                    {l.guarantor_seat >= 0 ? (
                      ...
                      <button type="button" className="btn btn-mini btn-secondary" onClick={() => handleGuarant(l)}>
                        {t('virtualCity.loan.addGuarantor' as TKey)}
                      </button>
                    )}
                    {l.overdue && <span className="virtualCity-badge virtualCity-badge--overdue">{t('virtualCity.loan.overdueBadge' as TKey)}</span>}
                  </td>
                  <td>
                    <button type="button" className="btn btn-mini btn-primary" onClick={() => handleRepay(l, false)}>
                      {t('virtualCity.loan.repayBtn' as TKey)}
                    </button>
                    <button type="button" className="btn btn-mini btn-secondary" onClick={() => handleRepay(l, true)}>
                      {t('virtualCity.loan.repayFull' as TKey)}
                    </button>
                  </td>
                </tr>
              ))}
          ...
          {myBorrowed.length === 0 && <p className="virtualCity-table__empty">{t('virtualCity.loan.empty' as TKey)}</p>}

          {myLent.length > 0 && <div className="virtualCity-loanpanel__section-title">{t('virtualCity.loan.lend' as TKey)}</div>}
          {myLent.length > 0 && (
            <table className="virtualCity-table">
              <thead>
                <tr>
              ...
              <tbody>
                {myLent.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td>¥{formatCny(l.principal_cny)}</td>
                    <td className="virtualCity-num--pos">¥{formatCny(l.balance_cny)}</td>
                    <td>{l.months_left}月</td>
                  </tr>
                ))}
              </tbody>
