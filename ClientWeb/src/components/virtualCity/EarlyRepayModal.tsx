...
interface Props {
  ...
  mortgageLoans: VirtualCityLoan[];
  /** 当前理财收益率（年化，小数；用于机会成本提示）。 */
  investYield: number;
  /** 提交回调。 */
  onSubmit: (action: VirtualCityAction) => void;
  /** 关闭回调（维持不变）。 */
  onDismiss: () => void;
}
...
function estimatePenalty(loan: VirtualCityLoan): number {
...
function estimateSavedInterest(loan: VirtualCityLoan): number {
...
export function EarlyRepayModal({ open, mortgageLoans, investYield, onSubmit, onDismiss }: Props) {
  ...
  const handleConfirm = () => {
    if (!selectedLoan) {
      setFormError(t('virtualCity.action.noLoan' as TKey));
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="virtualCity-earlyrepay virtualCity-modal__body">
        {/* 机会成本提示 */}
        {showTriggerHint && (
          <div className="virtualCity-earlyrepay__trigger">
            <span className="virtualCity-earlyrepay__trigger-icon" aria-hidden="true">📊</span>
            <span>
        ...
        {mortgageLoans.length > 1 && mode === 'choose' && (
          <label className="virtualCity-earlyrepay__row">
            ...
            >
              {mortgageLoans.map((l) => (
                <option key={l.id} value={l.id}>
                  {l.id} · {t('virtualCity.loan.balance' as TKey)} ¥{formatCny(l.balance)} · {formatPct(l.annual_rate)}
                </option>
              ))}
            </select>
          </label>
        ...
        {/* 贷款详情 */}
        <div className="virtualCity-earlyrepay__details">
          <div className="virtualCity-earlyrepay__detail-row">
            <span>{t('earlyrepay.principal' as TKey)}</span>
            <b>¥{formatCny(selectedLoan.principal)}</b>
          </div>
          <div className="virtualCity-earlyrepay__detail-row">
            <span>{t('virtualCity.loan.balance' as TKey)}</span>
            <b>¥{formatCny(selectedLoan.balance)}</b>
          </div>
          <div className="virtualCity-earlyrepay__detail-row">
            <span>{t('virtualCity.loan.rate' as TKey)}</span>
            <b>{formatPct(selectedLoan.annual_rate, 2)}</b>
          </div>
          <div className="virtualCity-earlyrepay__detail-row">
            <span>{t('virtualCity.loan.monthly' as TKey)}</span>
            <b>¥{formatCny(selectedLoan.monthly_payment)} × {selectedLoan.months_left}</b>
          </div>
          <div className="virtualCity-earlyrepay__detail-row">
            <span>{t('earlyrepay.penalty' as TKey)}</span>
            <b className={penalty > 0 ? 'virtualCity-num--neg' : ''}>
              ¥{formatCny(penalty)}
            </b>
          </div>
          <div className="virtualCity-earlyrepay__detail-row">
            <span>{t('earlyrepay.savedInterest' as TKey)}</span>
            <b className="virtualCity-num--pos">¥{formatCny(savedInterest)}</b>
          </div>
        </div>

        {/* 部分还款金额 */}
        {mode === 'confirm-partial' && (
          <div className="virtualCity-earlyrepay__partial-info">
            <p>
              {t('earlyrepay.partialAmount' as TKey)}：<b>¥{formatCny(partialAmount)}</b>
            </p>
            <p>
              {t('earlyrepay.penalty' as TKey)}：<b className={partialPenalty > 0 ? 'virtualCity-num--neg' : ''}>¥{formatCny(partialPenalty)}</b>
            </p>
          </div>
        )}
        ...
        {/* 危险操作二次确认提示 */}
        {mode === 'confirm-full' && (
          <div className="virtualCity-earlyrepay__danger">
            <p className="virtualCity-earlyrepay__danger-title">
