// Package wealth — agent_sense.go: City-Human 感知与行动工具桥
// (2026-09-22 §CityHuman重构)。
//
// 契约: lag_docs/虚拟城市/已实现/12-CityHuman重构/虚拟城市-CityHuman-Agent合并与感知系统设计-v1.md §3/§4。
// 实现 wealthplayer.ToolRunner 的 See/Hear/Smell/Move/SpeakTo 五方法:
...
func (a *AgentRunner) ambianceForLocked(district string) wealthtypes.AmbianceBrief {
	...
	return wealthtypes.AmbianceBrief{District: district, Smells: smells, Sounds: sounds}
}

// cityAmbianceLocked 全城 16 区当月感官画像(锁内;CitySnapshotView 填充
// city.Snapshot.Ambiance 用)。
func (r *WealthRoom) cityAmbianceLocked() map[string]city.AmbianceTags {
...
func (a *AgentRunner) See(seat int) (*wealthtypes.SenseResult, error) {
	a.room.mu.Lock()
	...
	p := w.Players[seat]
	district := p.District
	res := &wealthtypes.SenseResult{District: district}
	...
	for s, pp := range w.Players {
		...
		res.People = append(res.People, wealthtypes.NeighborBrief{
...
func (a *AgentRunner) Hear(seat int) (*wealthtypes.SenseResult, error) {
	a.room.mu.Lock()
	if err := a.senseGateLocked(seat); err != nil {
		a.room.mu.Unlock()
		return nil, err
...
func (a *AgentRunner) Smell(seat int) (*wealthtypes.SenseResult, error) {
	a.room.mu.Lock()
	if err := a.senseGateLocked(seat); err != nil {
		a.room.mu.Unlock()
		return nil, err
	...
	district := p.District
	ab := a.ambianceForLocked(district)
	res := &wealthtypes.SenseResult{District: district, Smells: ab.Smells, Sounds: ab.Sounds}
...
func firstNeighborName(people []wealthtypes.NeighborBrief) string {
	if len(people) == 0 {
		return "附近无人"
	}
...
func (a *AgentRunner) SpeakTo(seat int, targetSeat int, text string) error {
	...
	if targetSeat == seat || targetSeat < 0 || targetSeat >= MaxSeats {
		a.room.mu.Unlock()
		return errcode.Code(errcode.ErrWealthWhisperTarget)
	}
	...
	if !ok || wsp == nil {
		a.room.mu.Unlock()
		return errcode.CodeMsg(errcode.ErrWealthWhisperTarget, "whisper channel unavailable")
	}
