// Package wealth — agent_context_batch20_test.go: 三块 GameContext 注入小节
...
func TestSideMarketBrief_BudgetAndContent(t *testing.T) {
	w := briefWorld(t, 2)
	r := NewWealthRoom("room-sm", 3000, 5, 4)
	r.World = w
...
func TestElectionBrief(t *testing.T) {
	w := briefWorld(t, 2)
	r := NewWealthRoom("room-el", 3000, 5, 4)
	r.World = w
...
func TestBuildContextForAgent_Batch20Fields(t *testing.T) {
	...
	r := NewWealthRoom("room-all", 3000, 5, 4)
	r.World = w
