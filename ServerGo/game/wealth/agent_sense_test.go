// Package wealth — agent_sense_test.go: City-Human 感知与行动工具引擎侧单测
...
func TestActMove_CrossDistrictModes(t *testing.T) {
	...
	p2 := w.Players[1]
	_ = p2
	if _, e := w.ApplyAction(1, Action{Type: ActMove, District: "suburb", Mode: "rocket"}); e == nil || e.Code != errcode.ErrWealthSenseInvalid {
		t.Errorf("unknown mode: got %v, want 35100", e)
	}
	// 未知城区 → 35100。
	if _, e := w.ApplyAction(1, Action{Type: ActMove, District: "moon", Mode: "bus"}); e == nil || e.Code != errcode.ErrWealthSenseInvalid {
		t.Errorf("unknown district: got %v, want 35100", e)
	}
	// 区内移动不许用交通工具 → 35100。
	if _, e := w.ApplyAction(1, Action{Type: ActMove, Mode: "taxi"}); e == nil || e.Code != errcode.ErrWealthSenseInvalid {
...
func beginDecisionFor(t *testing.T, r *WealthRoom, seat int) *AgentRunner {
	t.Helper()
...
func TestSpeakTo_WhisperDirected(t *testing.T) {
	...
	} else if e, ok := err.(*errcode.Error); !ok || e.Code != errcode.ErrWealthWhisperTarget {
...
func TestSpeak_MonthlyLimitTwo(t *testing.T) {
	...
	} else if e, ok := err.(*errcode.Error); !ok || e.Code != errcode.ErrWealthSenseLimit {
