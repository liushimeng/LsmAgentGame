// Package wealth — interbank_cd.go: 同业存单市场(银行间短期融资工具)
...
func (w *World) BuyCD(seat int, cdID string, faceWan float64) (*CDHolding, *errcode.Error) {
	...
	if faceWan < CDBuyMinFaceWan {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "cd face_wan must be >= 1")
	}
	if w.CDMarket == nil {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "cd market not initialized")
	}
	cd := w.CDMarket.cdByID(cdID)
	if cd == nil {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "cd not found: "+cdID)
	}
...
func (w *World) SellCD(seat int, holdingID string) (int64, *errcode.Error) {
	if w.CDMarket == nil {
		return 0, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "cd market not initialized")
	}
	idx := -1
	...
	if idx < 0 {
		return 0, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "cd holding not found: "+holdingID)
	}
