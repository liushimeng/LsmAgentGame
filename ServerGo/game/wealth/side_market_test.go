// Package wealth — side_market_test.go: 副业竞争比价与定价战单测(批次20 文档2 §6)。
//
...
func TestSideTierGate_Matrix(t *testing.T) {
	...
	for _, c := range cases {
		...
		if e != nil && e.Code != errcode.ErrWealthGateFailed {
...
func TestAction_StartSideBusiness_Tier(t *testing.T) {
	...
	p.ActionBudget = 3
	if _, e = w.ApplyAction(0, Action{Type: ActStartSide, Kind: "delivery", Tier: 5}); e == nil || e.Code != errcode.ErrWealthGateFailed {
...
func TestAction_SetSidePrice(t *testing.T) {
	...
	if _, e := w.ApplyAction(1, Action{Type: ActSetSidePrice, Tier: PriceTierLow}); e == nil ||
		e.Code != errcode.ErrWealthGateFailed {
	...
	w.Month++
	if _, e := w.ApplyAction(0, Action{Type: ActSetSidePrice, Tier: 9}); e == nil || e.Code != errcode.ErrWealthGateFailed {
