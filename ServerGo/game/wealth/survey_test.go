// Package wealth — survey_test.go: 社会调研生命周期单测
...
func mustLaunch(t *testing.T, r *WealthRoom, q string, opts ...string) *Survey {
	t.Helper()
...
func TestSurvey_LaunchValidation(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty question", "", []string{"A", "B"}, errcode.ErrWealthSurveyOptionsInvalid},
		{"one option", "问", []string{"A"}, errcode.ErrWealthSurveyOptionsInvalid},
		{"seven options", "问", []string{"1", "2", "3", "4", "5", "6", "7"}, errcode.ErrWealthSurveyOptionsInvalid},
		{"blank option", "问", []string{"A", "  "}, errcode.ErrWealthSurveyOptionsInvalid},
	}
	for _, c := range cases {
...
func TestSurvey_LaunchLimits(t *testing.T) {
	...
	if _, e := r.LaunchSurvey("Q2?", []string{"A", "B"}); e == nil || e.Code != errcode.ErrWealthSurveyOpenExists {
		t.Errorf("second open: got %v, want %d", e, errcode.ErrWealthSurveyOpenExists)
	}
	// 关闭第 1 个;同月再发 → 35018(每月 1 个)。
	w.CloseAndAggregate(w.Surveys[0])
	if _, e := r.LaunchSurvey("Q2?", []string{"A", "B"}); e == nil || e.Code != errcode.ErrWealthSurveyMonthlyLimit {
		t.Errorf("same-month relaunch: got %v, want %d", e, errcode.ErrWealthSurveyMonthlyLimit)
	}
	...
	w.Month++
	if _, e := r.LaunchSurvey("Q21?", []string{"A", "B"}); e == nil || e.Code != errcode.ErrWealthSurveyMonthlyLimit {
		t.Errorf("21st survey: got %v, want %d (cap 20)", e, errcode.ErrWealthSurveyMonthlyLimit)
	}
}
...
func TestSurvey_AnswerAggregate(t *testing.T) {
	...
	if e := a2.AnswerSurvey(2, sv.ID, 5, "x"); e == nil || errCodeOf(e) != errcode.ErrWealthSurveyOptionsInvalid {
		t.Errorf("option idx 5: got %v, want %d", e, errcode.ErrWealthSurveyOptionsInvalid)
	}
	if e := a2.AnswerSurvey(2, sv.ID, 0, "跟随大众"); e != nil {
		t.Fatalf("answer 2: %v", e)
	}
	// 重复回答 → 35019。
	if e := a0.AnswerSurvey(0, sv.ID, 1, "改主意"); e == nil || errCodeOf(e) != errcode.ErrWealthSurveyNotFound {
		t.Errorf("duplicate answer: got %v, want %d", e, errcode.ErrWealthSurveyNotFound)
	}
	// 人类座位(非 bot)不可答 → 35005。
	r.MuLock()
	r.BotSeats[3] = false
	r.MuUnlock()
	if e := NewAgentRunner(r, 3).AnswerSurvey(3, sv.ID, 0, "人类"); e == nil || errCodeOf(e) != errcode.ErrWealthPlayerInactive {
		t.Errorf("human seat answer: got %v, want %d", e, errcode.ErrWealthPlayerInactive)
	}
	...
	// 关闭后回答 → 35019。
	if e := a1.AnswerSurvey(1, sv.ID, 0, "补答"); e == nil || errCodeOf(e) != errcode.ErrWealthSurveyNotFound {
		t.Errorf("answer after close: got %v, want %d", e, errcode.ErrWealthSurveyNotFound)
	}
	// 幂等:重复关闭不重复聚合。
	total := res.Total
	w.CloseAndAggregate(sv)
