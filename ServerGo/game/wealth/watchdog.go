// Package wealth — watchdog.go: 月窗口 watchdog + Agent 超时兜底(2026-09-14 §财商流P0)。
//
// 契约: 后端架构文档 §14。月窗口 watchdog 监测 tick 卡死,Agent 超时
// 由 wealthplayer.Agent 的 ctx(decisionTimeout)+ room.runLoop 的 settleCh
...
func (r *WealthRoom) StartWatchdog(onStuck func()) {
	go func() {
		...
		for {
			...
			case <-t.C:
				...
				r.mu.Unlock()
				if stuck {
					logger.L().Error("wealth room stuck beyond watchdog threshold, forcing settle",
