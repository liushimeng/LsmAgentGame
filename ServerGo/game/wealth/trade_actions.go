// Package wealth — trade_actions.go: P2 交易类动作实现(2026-09-16 §财商流P2)。
//
// 封装 WS 动作帧(game.wealth_action)对应的 12 个交易动作;人类按钮与 Agent 工具
...
func (w *World) tradeListAsset(p *Player, ta TradeAction) (string, *errcode.Error) {
	if ta.Asset == "" {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "asset kind required")
	}
	// 查找资产。
	var snap Asset
	idx := -1
	...
	if idx < 0 {
		return "", errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "asset not found")
	}
...
func (w *World) tradeNegotiateStart(p *Player, ta TradeAction) (string, *errcode.Error) {
	if ta.ListingID == "" {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "listing_id required")
	}
...
func (w *World) tradeBidAuction(p *Player, ta TradeAction) (string, *errcode.Error) {
	if ta.AuctionID == "" {
		return "", errcode.CodeMsg(errcode.ErrWealthAuctionNotFound, "auction_id required")
	}
...
func (w *World) tradeStartAuction(p *Player, ta TradeAction) (string, *errcode.Error) {
	...
	if ta.Asset == "" {
		return "", errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "asset kind required")
	}
	// 查找资产快照。
	var snap Asset
	seen := 0
