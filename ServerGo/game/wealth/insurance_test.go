// Package wealth — insurance_test.go: 商业保险与风险转移引擎单测
...
func TestInsurance_BuyValidation(t *testing.T) {
	...
	if e := buy("unit_link"); e == nil || e.Code != errcode.ErrWealthInsuranceKindInvalid {
		t.Errorf("kind invalid: got %v, want 35037", e)
	}
	if e := buy(InsAccident); e != nil {
		t.Fatalf("first buy should pass: %v", e)
	}
	if e := buy(InsAccident); e == nil || e.Code != errcode.ErrWealthInsuranceExists {
	...
	w.Month = (56-25)*12 + 1
	if e := buy(InsCriticalIllness); e == nil || e.Code != errcode.ErrWealthInsuranceAgeGate {
		t.Errorf("age 56 buy: got %v, want 35040", e)
	}
	// 现金不足首月保费 → 35007(回到 25 岁,避免年龄门先行短路)。
	w.Month = 1
	p.Cash = 10
	if e := buy(InsCriticalIllness); e == nil || e.Code != errcode.ErrWealthInsufficientCash {
		t.Errorf("insufficient cash: got %v, want 35007", e)
	}
	// insurance_enabled=false → 35041(投保与退保均拒)。
	wd := insWorld(13, false)
	if _, e := wd.ApplyAction(0, Action{Type: ActBuyInsurance, Kind: InsAccident}); e == nil || e.Code != errcode.ErrWealthInsuranceDisabled {
		t.Errorf("disabled buy: got %v, want 35041", e)
	}
	if _, e := wd.ApplyAction(0, Action{Type: ActCancelInsurance, Kind: InsAccident}); e == nil || e.Code != errcode.ErrWealthInsuranceDisabled {
...
func TestInsurance_CancelZeroValue(t *testing.T) {
	...
	// 重复退保 → 35039。
	if _, e := w.ApplyAction(0, Action{Type: ActCancelInsurance, Kind: InsMedicalMillion}); e == nil || e.Code != errcode.ErrWealthInsuranceNotFound {
