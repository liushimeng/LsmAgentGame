// Package wealth — listing_test.go: P2 挂单簿 + 议价 + 借贷合约单测
// (2026-09-16 §财商流P2)。
package wealth
...
func TestListingSelfTradeRejected(t *testing.T) {
	...
	if err.Code != errcode.ErrWealthSelfTrade {
		t.Fatalf("self-trade error code: got %d, want %d", err.Code, errcode.ErrWealthSelfTrade)
	}
}
...
func TestLoanRateValidation(t *testing.T) {
	...
	if err.Code != errcode.ErrWealthLoanRateInvalid {
		t.Fatalf("rate error code: got %d, want %d", err.Code, errcode.ErrWealthLoanRateInvalid)
	}
...
func TestLoanCreditValidation(t *testing.T) {
	...
	if err.Code != errcode.ErrWealthLoanNoCredit {
		t.Fatalf("credit error code: got %d, want %d", err.Code, errcode.ErrWealthLoanNoCredit)
	}
}
...
func TestListingFull(t *testing.T) {
	...
	if err.Code != errcode.ErrWealthListingFull {
		t.Fatalf("listing full error code: got %d, want %d", err.Code, errcode.ErrWealthListingFull)
	}
}
