// Package wealth — auction.go: 四种拍卖引擎(2026-09-16 §财商流P2)。
//
// 契约: lag_docs/财商流游戏/已实现/06-P2交易系统/财商流游戏-P2-玩家间交易与财富流动系统-v1.md §4。
...
func (w *World) StartAuction(sellerSeat int, aType AuctionType, asset Asset, startPrice int64) (*Auction, *errcode.Error) {
	...
	if startPrice < 100 {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "start_price must be >= 100")
	}
	// 校验资产存在。
	if !w.assetExists(seller, asset) {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "asset not in seller portfolio")
	}
...
func (w *World) PlaceBid(bidderSeat int, auctionID string, amount int64) *errcode.Error {
	...
	if a == nil {
		return errcode.Code(errcode.ErrWealthAuctionNotFound)
	}
	if a.Status != AuctionStatusActive {
		return errcode.Code(errcode.ErrWealthAuctionEnded)
	}
	...
	case AuctionEnglish:
		...
		if amount < minNext {
			return errcode.CodeMsg(errcode.ErrWealthBidTooLow, fmt.Sprintf("bid must be >= %d", minNext))
		}
		a.Seq++
	...
	case AuctionDutch:
		...
		if amount >= currentPrice {
			...
			return errcode.CodeMsg(errcode.ErrWealthBidTooLow, fmt.Sprintf("dutch price is %d", currentPrice))
		}
	default:
		return errcode.CodeMsg(errcode.ErrWealthListingInvalid, "use PlaceSealedBid for sealed/vickrey auctions")
	}
	return nil
}
...
func (w *World) PlaceSealedBid(bidderSeat int, auctionID string, amount int64) *errcode.Error {
	...
	if a.Type != AuctionSealed && a.Type != AuctionVickrey {
		return errcode.CodeMsg(errcode.ErrWealthListingInvalid, "not a sealed-bid auction")
	}
	if amount < a.StartPrice {
		return errcode.Code(errcode.ErrWealthBidTooLow)
	}
...
func (w *World) finalizeAuction(a *Auction) (string, *errcode.Error) {
	...
	if idx < 0 {
		a.Status = AuctionStatusUnsold
		return "", errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "auction asset no longer exists")
	}
