// Package wealth — insurance_e2e_test.go: P1-4 商业保险引擎级 e2e 跨模块验证
...
func TestInsuranceE2E_TwelveSeats_26Months(t *testing.T) {
	...
	cs := BuildClientState("room-ins-e2e", 4, w, seats, nicks, bots, models, [MaxSeats]BotTranscript{}, 0, 0)
...
func TestInsuranceE2E_AgentToolPath(t *testing.T) {
	...
	if e, ok := err.(*errcode.Error); !ok || e.Code != errcode.ErrWealthInsuranceExists {
...
//     ← trySettle(r.mu 持有中)。WS 动作路径(applyWealthAction → MuLock →
...
//   - 已知存量注意项(非 P1-4 引入):ws 层 broadcastWealthState 经 Engine()
...
func TestInsuranceE2E_ConcurrentActionSettleDeathRace(t *testing.T) {
	...
	wg.Add(1)
	go func() { // WS 动作路径同款(MuLock + ApplyAction,仿 applyWealthAction)
		defer wg.Done()
		for i := 0; i < 200; i++ {
			seat := 1 + i%6
