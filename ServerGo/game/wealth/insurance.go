// Package wealth — insurance.go: 商业保险与风险转移引擎(2026-09-19 §财商流P1-4)。
//
// 契约: lag_docs/财商流游戏/已实现/10-P1保险系统/财商流游戏-P1-商业保险与风险转移引擎-v1.md。
...
func (w *World) HandleDeath(seat int, cause string) {
	...
	// 【代际引擎对接点】《财商流游戏-P1-代际财富转移引擎-v1.md》§3 的
...
func (w *World) actBuyInsurance(p *Player, a Action) (string, *errcode.Error) {
	if !w.InsuranceEnabled {
		return "", errcode.Code(errcode.ErrWealthInsuranceDisabled) // 35041
	}
	if p.ActionBudget <= 0 {
		return "", errcode.Code(errcode.ErrWealthActionBudgetExhausted) // 35006
	}
	def, ok := policyDefs[a.Kind]
	if !ok {
		return "", errcode.Code(errcode.ErrWealthInsuranceKindInvalid) // 35037
	}
	if pol := p.Policies[a.Kind]; pol != nil && pol.Active {
		return "", errcode.Code(errcode.ErrWealthInsuranceExists) // 35038
	}
	if w.Age() > insuranceAgeGate {
		return "", errcode.Code(errcode.ErrWealthInsuranceAgeGate) // 35040
	}
	annual := priceAnnual(def, w.Age(), 1.0)
	monthly := monthlyPremium(annual)
	if p.Cash < monthly {
		return "", errcode.Code(errcode.ErrWealthInsufficientCash) // 35007
	}
	if p.Policies == nil {
...
func (w *World) actCancelInsurance(p *Player, a Action) (string, *errcode.Error) {
	...
	if pol == nil || !pol.Active {
		return "", errcode.Code(errcode.ErrWealthInsuranceNotFound) // 35039
	}
	pol.Active = false
	pol.GraceActive = false
