// Package wealth — listing.go: 挂单簿 + 议价会话 + 玩家间借贷合约(2026-09-16 §财商流P2)。
//
// 契约: lag_docs/财商流游戏/已实现/06-P2交易系统/财商流游戏-P2-玩家间交易与财富流动系统-v1.md §3/§5/§6。
...
func (w *World) CreateListing(seat int, lType ListingType, payload ListingPayload, askCNY int64) (*Listing, *errcode.Error) {
	p := w.Players[seat]
	if p == nil || !p.Alive {
		return nil, errcode.Code(errcode.ErrWealthPlayerInactive)
	}
	if w.ListingBook.activeListingsBySeat(seat) >= maxListingsPerSeat {
		return nil, errcode.Code(errcode.ErrWealthListingFull)
	}
	...
	case ListingAsset:
		if payload.Asset == nil {
			return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "asset payload required")
		}
	...
	case ListingBuy:
		...
		if askCNY < 1000 {
			return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "buy listing ask_cny must be >= 1000")
		}
	default:
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "unsupported listing type for CreateListing")
	}
...
func (w *World) CancelListing(seat int, listingID string) *errcode.Error {
	...
	if l == nil {
		return errcode.Code(errcode.ErrWealthListingNotFound)
	}
	if l.Seat != seat {
		return errcode.Code(errcode.ErrWealthNoPrivilege)
	}
	if l.Status != ListingStatusOpen && l.Status != ListingStatusNegotiating {
		return errcode.CodeMsg(errcode.ErrWealthListingInvalid, "cannot cancel listing in status "+l.Status)
	}
...
func (w *World) NegotiateStart(proposerSeat int, listingID string, firstOffer int64) (*NegotiateSession, *errcode.Error) {
	...
	l := w.ListingBook.Listings[listingID]
	if l == nil {
		return nil, errcode.Code(errcode.ErrWealthListingNotFound)
	}
	if l.Status != ListingStatusOpen {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "listing not open")
	}
	if l.Seat == proposerSeat {
		return nil, errcode.Code(errcode.ErrWealthSelfTrade)
	}
	if firstOffer < 0 {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "first_offer must be >= 0")
	}
...
func (w *World) NegotiateRespond(seat int, negID string, action string, offerCNY int64, comment string) (string, *errcode.Error) {
	...
	if neg == nil {
		return "", errcode.Code(errcode.ErrWealthNegotiateNotFound)
	}
	if neg.Status != NegotiateStatusActive {
		return "", errcode.CodeMsg(errcode.ErrWealthNegotiateNotFound, "negotiate not active")
	}
	// 轮次判定:LastOfferBy 的对方必须响应。
	if neg.LastOfferBy == seat {
		return "", errcode.Code(errcode.ErrWealthNotYourTurn)
	}
	p := w.Players[seat]
	if p == nil || !p.Alive {
	...
	case NegotiateActCounter, NegotiateActOffer:
		if offerCNY < 0 {
			return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "counter offer must be >= 0")
		}
		...
		return "还价已提交", nil
	default:
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "unknown negotiate action: "+action)
	}
}
...
func (w *World) executeDeal(neg *NegotiateSession) (string, *errcode.Error) {
	...
	default:
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "unknown listing type")
	}
}
...
func (w *World) dealAsset(l *Listing, neg *NegotiateSession, price int64) (string, *errcode.Error) {
	...
	if idx < 0 {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "seller asset no longer exists")
	}
...
func (w *World) dealBuy(l *Listing, neg *NegotiateSession, price int64) (string, *errcode.Error) {
	...
	if seller.SideBusiness == nil {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "seller has no side business to transfer")
	}
	if buyer.Cash < price {
		return "", errcode.Code(errcode.ErrWealthInsufficientCash)
	}
...
func (w *World) dealLoan(l *Listing, neg *NegotiateSession, price int64) (string, *errcode.Error) {
	loan := l.Payload.Loan
	if loan == nil {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "loan payload required")
	}
	...
	default:
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "not a loan listing")
	}
...
func (w *World) CreateLoanListing(seat int, direction string, principal int64, rate float64, termN int, needGuarantee bool) (*Listing, *errcode.Error) {
	...
	if direction != "lend" && direction != "borrow" {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "direction must be lend|borrow")
	}
	if principal < p2pMinPrincipal {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "principal must be >= 1000")
	}
	if termN < p2pMinTermN || termN > p2pMaxTermN {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "term_n must be in [1,60]")
	}
	// 利率校验:月利率 0.3%-3.6%。
	if rate < p2pMinRatePerMonth || rate > p2pMaxRatePerMonth {
		return nil, errcode.Code(errcode.ErrWealthLoanRateInvalid)
	}
	// 借入方需信用分 ≥ 500。
	if direction == "borrow" && p.CreditScore < 500 {
		return nil, errcode.Code(errcode.ErrWealthLoanNoCredit)
	}
...
func (w *World) AcceptLoan(seat int, listingID string) (*P2PLoan, *errcode.Error) {
	...
	if l.Type != ListingLoanOfr && l.Type != ListingLoanReq {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "not a loan listing")
	}
	loan := l.Payload.Loan
	if loan == nil {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "loan payload required")
	}
	// 借入方信用校验。
	var lenderSeat, borrowerSeat int
	switch l.Type {
	...
	lender := w.Players[lenderSeat]
	if lender.Cash < loan.PrincipalCNY {
		return nil, errcode.Code(errcode.ErrWealthInsufficientCash)
	}
...
func (w *World) RepayLoan(seat int, loanID string, amount int64) (string, *errcode.Error) {
	...
	if loan == nil {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "loan not found")
	}
	if loan.BorrowerSeat != seat {
		return "", errcode.Code(errcode.ErrWealthNoPrivilege)
	}
	if loan.MonthsLeft <= 0 || loan.BalanceCNY <= 0 {
		return "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "loan already settled")
	}
...
func (w *World) AddGuarantor(loanID string, guarantorSeat int) *errcode.Error {
	...
	if loan.GuarantorSeat != -1 {
		return errcode.Code(errcode.ErrWealthGuarantorConflict)
	}
	...
	if g.CreditScore < 600 {
		return errcode.Code(errcode.ErrWealthLoanNoCredit)
	}
...
func (w *World) SellInfo(seat int, category string, title string, detail string, minBid int64) (*Listing, *errcode.Error) {
	...
	if category != "market" && category != "intel" && category != "personal" {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "category must be market|intel|personal")
	}
	if title == "" {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "title required")
	}
	if minBid < 100 {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "min_bid must be >= 100")
	}
	listing := &Listing{
		Type:    ListingInfo,
		Seat:    seat,
...
func (w *World) BidInfo(seat int, listingID string, bidCNY int64) *errcode.Error {
	...
	if l.Type != ListingInfo || l.Status != ListingStatusOpen {
		return errcode.CodeMsg(errcode.ErrWealthListingInvalid, "not an open info listing")
	}
	if l.Seat == seat {
		return errcode.Code(errcode.ErrWealthSelfTrade)
	}
	if bidCNY < l.Payload.InfoOffer.MinBidCNY {
		return errcode.Code(errcode.ErrWealthBidTooLow)
	}
...
func (w *World) RevealInfo(listingID string) (int, string, *errcode.Error) {
	l := w.ListingBook.Listings[listingID]
	if l == nil {
		return -1, "", errcode.Code(errcode.ErrWealthListingNotFound)
	}
	if l.Type != ListingInfo {
		return -1, "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "not an info listing")
	}
	if l.Status != ListingStatusOpen && l.Status != ListingStatusExpired {
		return -1, "", errcode.CodeMsg(errcode.ErrWealthListingInvalid, "listing not revealable")
	}
	if len(l.Payload.InfoOffer.Bids) == 0 {
		l.Status = ListingStatusExpired
		return -1, "", errcode.Code(errcode.ErrWealthInfoNotFound)
	}
	...
	if bidder == nil || !bidder.Alive || seller == nil {
		l.Status = ListingStatusExpired
		return -1, "", errcode.Code(errcode.ErrWealthPlayerInactive)
	}
	if bidder.Cash < maxBid {
		// 现金不足 → 流标。
		l.Status = ListingStatusExpired
		return -1, "", errcode.Code(errcode.ErrWealthInsufficientCash)
	}
