// Package wealth — short_selling.go: 融券做空子系统(2026-09-21 §城市扩张v2.12 阶段6)。
//
...
func (w *World) OpenShort(seat int, symbol string, shares int64) (*ShortPosition, *errcode.Error) {
	...
	if symbol != ShortSymbol {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "short symbol must be stock_index")
	}
	if shares < ShortMinShares {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid,
			fmt.Sprintf("short shares must be >= %d", ShortMinShares))
	}
	if w.ShortBook == nil {
		return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "short book not initialized")
	}
	// R6-1① 单只融券余额 ≤ 流通市值 5%(份数口径:流通股本 × 5%)。
	if existing := w.ShortBook.openShortShares(symbol); existing+shares > ShortFloatCapShares() {
		return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid,
...
func (w *World) CloseShort(seat int, posID string) (*ShortPosition, *errcode.Error) {
	...
	for _, pos := range w.ShortBook.Positions {
		...
		if pos.Status != ShortStatusOpen {
			return nil, errcode.CodeMsg(errcode.ErrWealthListingInvalid, "short position already closed: "+posID)
		}
	...
	return nil, errcode.CodeMsg(errcode.ErrWealthAssetInvalid, "short position not found: "+posID)
}
