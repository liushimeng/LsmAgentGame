// Package virtual_city — room_city_test.go: 城市背景层房间接线测试
...
func TestRegisterBotSeats_PoolSeatPlaceholderNickname(t *testing.T) {
	r := NewVirtualCityRoom("room-nick", 3000, 7, 4)
	r.RegisterBotSeats(
...
func TestStart_CityDeterministicPerSeed(t *testing.T) {
	mk := func() *VirtualCityRoom {
		r := NewVirtualCityRoom("room-det", 3000, 555, 4)
		botUsers := make(map[int]string, 12)
		for seat := 0; seat < 12; seat++ {
			botUsers[seat] = "b" + string(rune('a'+seat))
		}
...
func TestCurrentCPI_Default(t *testing.T) {
	r := NewVirtualCityRoom("room-cpi", 3000, 1, 4)
	r.mu.Lock()
	defer r.mu.Unlock()
...
func TestCityRng_IndependentFromEngineRng(t *testing.T) {
	mk := func() int64 {
		r := NewVirtualCityRoom("room-rng", 3000, 999, 4)
		botUsers := make(map[int]string, 12)
		for seat := 0; seat < 12; seat++ {
			botUsers[seat] = "b" + string(rune('a'+seat))
		}
...
// 建房链路透传:VirtualCityRoomOptions.ResidentCount 经 ApplyRoomOptions 落到房间
// (service virtualCityRoomConfigurer → Manager.ApplyRoomOptions 同路径)。
func TestManager_ApplyRoomOptions_ResidentCount(t *testing.T) {
	m := NewManager(Config{MonthMs: 3000}, nil)
	m.ApplyRoomOptions("room-rc", &VirtualCityRoomOptions{MonthMs: 5000, Seed: 42, ResidentCount: 12345})
	...
	// 负数防御为 0。
	m.ApplyRoomOptions("room-rc-neg", &VirtualCityRoomOptions{ResidentCount: -7})
