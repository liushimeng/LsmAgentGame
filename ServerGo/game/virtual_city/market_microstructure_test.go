// Package virtual_city — market_microstructure_test.go: 股票交易微观结构单测
// (批次20 文档3 B5)。
//
...
func TestStockT1_BuyLockAndThaw(t *testing.T) {
	...
	if _, e := w.ApplyAction(0, Action{Type: ActSellAsset, Asset: AssetStockIndex, Units: 1}); e == nil ||
		e.Code != errcode.ErrVirtualCityStockT1Locked {
...
func TestStockCircuitBreaker_TriggerScopeExpiry(t *testing.T) {
	...
	if _, e := w.ApplyAction(0, Action{Type: ActBuyAsset, Asset: AssetStockIndex, AmountCNY: 5000}); e == nil ||
		e.Code != errcode.ErrVirtualCityMarketCircuitBreak {
...
func TestErrcodes_Microstructure(t *testing.T) {
	if errcode.ErrVirtualCityMarketCircuitBreak != 35043 || errcode.ErrVirtualCityStockT1Locked != 35044 {
		t.Fatalf("codes: got %d/%d, want 35043/35044", errcode.ErrVirtualCityMarketCircuitBreak, errcode.ErrVirtualCityStockT1Locked)
	}
	for _, c := range []int{errcode.ErrVirtualCityMarketCircuitBreak, errcode.ErrVirtualCityStockT1Locked} {
