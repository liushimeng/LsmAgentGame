// Package virtual_city — civic_election_test.go: 市长选举启用参数链与事件单测
...
// TestCivicElection_RoomApplyOptsMerge 建房 HTTP 链路(service.VirtualCityRoomOptions
...
func TestCivicElection_RoomApplyOptsMerge(t *testing.T) {
	...
	m := NewManager(Config{MonthMs: 3000}, nil)
	m.ApplyRoomOptions("room-el-2", &service.VirtualCityRoomOptions{CivicElectionEnabled: true})
	...
	m2.ApplyRoomOptions("room-el-3", &service.VirtualCityRoomOptions{})
	...
	r.applyOpts(&service.VirtualCityRoomOptions{CivicElectionEnabled: true})
	if !r.electionEnabled {
