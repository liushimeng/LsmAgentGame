...
func TestVirtualCityRoom_StartEmitsTwelveOpeningHooks(t *testing.T) {
...
func TestVirtualCityRoom_TranscriptMetadataUpdatesAndMarksInactive(t *testing.T) {
	r := newRoomWithSeats(t, MaxSeats)
	if e := r.Start(nil); e != nil {
		t.Fatalf("start: %v", e)
	}
	...
	NewAgentRunner(r, 10).RecordTranscript(10, vcplayer.BotTranscript{
...
func TestVirtualCityRoom_MonthTimeoutWritesObservableTranscript(t *testing.T) {
	r := newRoomWithSeats(t, MaxSeats)
	if e := r.Start(nil); e != nil {
		t.Fatalf("start: %v", e)
	}
...
func TestAgentRunner_ActionBroadcastsEventAndTranscript(t *testing.T) {
	...
	if got.LastToolInput != vcplayer.ToolSetConsumption {
		t.Fatalf("action transcript tool = %q, want %q", got.LastToolInput, vcplayer.ToolSetConsumption)
	}
}

func TestVirtualCityAgentPublishesDecisionTranscriptBeforeSubmit(t *testing.T) {
	var calls int32
	m := NewManager(Config{
	...
	if got.LastToolInput != vcplayer.ToolSubmitMonth || got.LastDecisionSummary == "" {
...
func TestVirtualCityRoom_FullAgentModeRejectsHumanWhileOpen(t *testing.T) {
	r := NewVirtualCityRoom("room-full-agent-open", 3000, 1, 4)
	...
	if err.Code != errcode.ErrVirtualCityFullAgentReject {
		t.Fatalf("join error code = %d, want %d", err.Code, errcode.ErrVirtualCityFullAgentReject)
	}
	if seat != -1 {
...
func TestManager_CreateRoomUsesPendingOptionsForRoomConfigLog(t *testing.T) {
	...
	m.ApplyRoomOptions("room-pending-config", &service.VirtualCityRoomOptions{
		MonthMs: 3000,
		Seed:    123,
	})
