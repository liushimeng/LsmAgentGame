...
func (l *Loader) buildIndex() {
	l.indexOnce.Do(func() {
		...
		if !l.available {
			logger.L().Warn("virtual_city profession docs pool unavailable, will fall back to synthetic cards",
...
func (l *Loader) runSelfCheckOnce() {
	l.selfCheckOnce.Do(func() {
		...
		if res.SuccessRate < selfCheckMinSuccessRate {
			logger.L().Error("virtual_city profession docs pool parse rate below threshold — 文档池卡形状可能已漂移,Draw 将大量回退合成卡",
				zap.String("root", l.root),
		...
		logger.L().Info("virtual_city profession docs pool self-check ok",
			zap.String("root", l.root),
			zap.Int("sampled", res.Sampled),
			zap.Float64("success_rate", res.SuccessRate),
			zap.Float64("valid_rate", res.ValidRate))
...
func (l *Loader) loadCard(rel string) (Card, error) {
	...
	l.mu.Unlock()
	if err != nil {
		logger.L().Warn("virtual_city profession card parse failed, skipped",
