...
func TestClientState_NoNullArrays(t *testing.T) {
	r := NewVirtualCityRoom("room-null", 3000, 7, 4)
	...
	for _, viewer := range []int{0, -1} {
		cs := BuildClientState("room-null", viewer, r.Engine(), r.SnapshotSeats(), r.SnapshotNicknames(), r.SnapshotBotSeats(), r.SnapshotModelKeys(), r.SnapshotTranscripts(), r.GameStartedAtUnix(), r.NextMonthAtUnix(), r.CityClockMs(), nil)
