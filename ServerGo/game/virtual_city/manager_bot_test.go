...
func TestManager_CreateRoom_NoDeadlock(t *testing.T) {
	...
	m.ApplyRoomOptions("room-x", &VirtualCityRoomOptions{MonthMs: 5000, Seed: 42})

	done := make(chan *VirtualCityRoom, 1)
	...
	// 已存在路径同样不得死锁。
	done2 := make(chan *VirtualCityRoom, 1)
...
// TestVirtualCityRoom_RegisterBotSeats_SeatsUsers 回归: 2026-09-14 P0 ——
// RegisterBotSeats 旧版不写 Seats[seat] 的 bot userID,Start 发卡跳过 bot、
// EnsureAgents 跳过,bot 永不上场。
func TestVirtualCityRoom_RegisterBotSeats_SeatsUsers(t *testing.T) {
	r := NewVirtualCityRoom("room-b", 3000, 7, 4)
	r.RegisterBotSeats(
...
func TestManager_CreateRoom_HydratedTenOrElevenBotsRestoreFullAgentMode(t *testing.T) {
	for _, count := range []int{MinSeats, MinSeats + 1} {
		t.Run(fmt.Sprintf("%dbots", count), func(t *testing.T) {
			...
			if err == nil || err.Code != errcode.ErrVirtualCityFullAgentReject {
				t.Fatalf("JoinGame error = %v, want ErrVirtualCityFullAgentReject", err)
			}
			if seat != -1 {
...
func TestManager_EnsureAgentsLockedVariant_ReentrantUnderWriteLock(t *testing.T) {
	...
	r := NewVirtualCityRoom("room-92a-reentrant", 3000, 7, 4)
	r.RegisterBotSeats(
