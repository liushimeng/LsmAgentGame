// Package virtual_city — agent_runner.go: ToolRunner 实现 + GameContext 构建
// (2026-09-14 §财商流P0)。
//
// 引擎桥:vcplayer.Agent 通过此工具桥调用 engine.ApplyAction
...
type AgentRunner struct {
	room *VirtualCityRoom
	seat int
}

// NewAgentRunner 构造工具桥。
func NewAgentRunner(r *VirtualCityRoom, seat int) *AgentRunner {
...
func (a *AgentRunner) SetConsumption(seat int, level int) error {
	return a.apply(seat, vcplayer.ToolSetConsumption, "", func() (string, error) {
...
func (a *AgentRunner) AnswerSurvey(seat int, surveyID string, optionIdx int, reason string) error {
	...
	if sv == nil || sv.Status != SurveyOpen {
		a.room.mu.Unlock()
		return errcode.Code(errcode.ErrVirtualCitySurveyNotFound) // 35019
	}
	if _, dup := sv.Answers[seat]; dup {
		a.room.mu.Unlock()
		return errcode.CodeMsg(errcode.ErrVirtualCitySurveyNotFound, "已回答过该调研")
	}
	if optionIdx < 0 || optionIdx >= len(sv.Options) {
		a.room.mu.Unlock()
		return errcode.Code(errcode.ErrVirtualCitySurveyOptionsInvalid) // 35016
	}
...
// RecordTranscript 实现 vcplayer.TranscriptSink。这是 bot_contexts 的
// 权威写入点:Agent 在月初、LLM 返回后、submit 前都会调用;旧月快照会被
// 拒绝,避免 3s 月窗下长 LLM 调用回写覆盖新月状态。
func (a *AgentRunner) RecordTranscript(seat int, transcript vcplayer.BotTranscript) {
	a.room.mu.Lock()
...
func (a *AgentRunner) ListAsset(seat int, assetIndex int, askCNY, minCNY int64) error {
	return a.apply(seat, vcplayer.ToolListAsset, "", func() (string, error) {
		...
		if assetIndex < 0 || assetIndex >= len(p.Assets) {
			return "", errcode.CodeMsg(errcode.ErrVirtualCityAssetInvalid, "asset_index 越界")
		}
...
func (r *VirtualCityRoom) buildCheckStateText(seat int) string {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.World == nil {
		return ""
...
func BuildContextForAgent(r *VirtualCityRoom, seat int) (*vctypes.GameContext, bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if r.World.InsuranceEnabled {
		for _, kind := range insuranceKindOrder {
			...
			me.Policies = append(me.Policies, vctypes.PolicyBrief{
				Kind:              pol.Kind,
	...
	// Peers 公开字段。
	peers := []vctypes.PeerBrief{}
	for s, pp := range r.World.Players {
		if pp == nil || s == seat {
			continue
		}
		nw := pp.NetWorth(r.World.Market)
		peers = append(peers, vctypes.PeerBrief{
	...
	// BotIdentity。
	botIdent := vctypes.BotIdentityBrief{
	...
	// P1: 央行快照 + 信贷约束参数。
	var cbSnapshot *vctypes.CentralBankSnapshot
	creditTightness, loanQuotaFactor := 0.0, 1.0
	if r.World.CB != nil {
		snap := r.World.CB.Snapshot()
		if snap != nil {
			cbSnapshot = &vctypes.CentralBankSnapshot{
	...
	return &vctypes.GameContext{
		...
		RoomID:          r.RoomID, GameKind: "virtual_city",
...
func surroundingsForLocked(r *VirtualCityRoom, p *Player) []vctypes.NeighborBrief {
	out := []vctypes.NeighborBrief{}
	for s, pp := range r.World.Players {
		if len(out) >= 8 {
			break
		}
		...
		out = append(out, vctypes.NeighborBrief{
...
func sideMarketBriefLocked(r *VirtualCityRoom, seat int) string {
	if r.World == nil {
		return ""
	}
	p := r.World.Players[seat]
...
func fiIndexFor(r *VirtualCityRoom, p *Player) float64 {
	return p.FIIndex(r.World.Market, r.World.Age())
}

// assetBriefFor 资产→vctypes 快照。
func assetBriefFor(r *VirtualCityRoom, a *Asset) vctypes.AssetBrief {
	b := vctypes.AssetBrief{
...
func assetPrice(r *VirtualCityRoom, a *Asset) float64 {
	switch {
...
func assetMonthlyFlow(r *VirtualCityRoom, a *Asset) int64 {
	switch {
...
// loanBriefFor 负债→vctypes 快照。
func loanBriefFor(l *Loan) vctypes.LoanBrief {
	return vctypes.LoanBrief{
...
func convertFlowItems(in []FlowItem) []vctypes.FlowDetailItem {
	if len(in) == 0 {
		return nil
	}
	out := make([]vctypes.FlowDetailItem, len(in))
	for i, f := range in {
		out[i] = vctypes.FlowDetailItem{Key: f.Key, AmountCNY: f.AmountCNY, Text: f.Text}
	}
	return out
}

func recentEvents(r *VirtualCityRoom, n int) []vctypes.EventBrief {
	events := r.World.RecentEvents(n)
	out := make([]vctypes.EventBrief, len(events))
	for i, e := range events {
		out[i] = vctypes.EventBrief{Month: e.Month, Type: e.Type, Seat: e.Seat, Text: e.Text}
	}
	return out
}

func ledgerRecent(r *VirtualCityRoom, seat, n int) []vctypes.LedgerBrief {
	ents := r.World.Ledger.SeatRecent(seat, n)
	out := make([]vctypes.LedgerBrief, len(ents))
	for i, e := range ents {
		out[i] = vctypes.LedgerBrief{
