...
// NewCustomTopicID 生成自定义辩题 ID("custom_<10位随机小写字母数字>")。
//
// §20260831-08:与内置题 ID 空间(classic_/new_/policy_/divergent_)天然不冲突。
func NewCustomTopicID() string {
	const letters = "abcdefghijklmnopqrstuvwxyz0123456789"
	b := make([]byte, 10)
	for i := range b {
		b[i] = letters[rand.Intn(len(letters))]
	}
	return "custom_" + string(b)
}
