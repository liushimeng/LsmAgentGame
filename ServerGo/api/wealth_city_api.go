...
//
//	GET /api/games/wealth/rooms/:id/city/residents          分页档案列表
...
//	GET /api/games/wealth/rooms/:id/city/residents/:cardId  单卡详情
//	    data: {resident: ResidentProfile};查不到 → 35042 ErrWealthResidentNotFound
//
...
// WealthCityAPI 是居民档案两端点的处理器(房间源 = wealthMgr,与
// WealthSurveyAPI 同装配模式)。
type WealthCityAPI struct {
	rooms WealthRoomSource
}

// NewWealthCityAPI 构造(rooms 由 main.go 注入 wealthMgr)。
func NewWealthCityAPI(rooms WealthRoomSource) *WealthCityAPI {
	return &WealthCityAPI{rooms: rooms}
}
...
// ListResidents 处理 GET /api/games/wealth/rooms/:id/city/residents。
func (a *WealthCityAPI) ListResidents(c *gin.Context) {
	roomID := c.Param("id")
	if a.rooms == nil {
...
// GetResident 处理 GET /api/games/wealth/rooms/:id/city/residents/:cardId。
func (a *WealthCityAPI) GetResident(c *gin.Context) {
	roomID := c.Param("id")
	if a.rooms == nil {
		c.JSON(http.StatusOK, gin.H{"code": errcode.ErrInternal, "message": "wealth manager not wired"})
		return
	...
	if !ok {
		c.JSON(http.StatusOK, gin.H{"code": errcode.ErrWealthResidentNotFound, "message": errcode.DefaultMessages[errcode.ErrWealthResidentNotFound]})
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"code":    errcode.OK,
