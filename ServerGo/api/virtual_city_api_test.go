...
func startAnchoredRoom(t *testing.T, roomID string, poolCards, residents int) (*virtual_city.Manager, *virtual_city.VirtualCityRoom) {
	t.Helper()
	m := virtual_city.NewManager(virtual_city.Config{MonthMs: 3000, Seed: 555}, nil)
...
func cityAPIRouter(rooms VirtualCityRoomSource) *gin.Engine {
	gin.SetMode(gin.TestMode)
	a := NewVirtualCityAPI(rooms)
	r := gin.New()
	g := r.Group("/api/games/virtual_city/rooms/:id")
...
// TestVirtualCityAPI_ListDetailNotFound(§12 G7)。
func TestVirtualCityAPI_ListDetailNotFound(t *testing.T) {
	if testing.Short() {
	...
	_, body := getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents")
	...
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents?limit=999")
	...
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents?limit=0")
	...
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents?offset=6")
	if got := len(body["data"].(map[string]any)["residents"].([]any)); got != residents-6 {
		t.Fatalf("offset page = %d, want %d", got, residents-6)
	}

	// ④ q 搜索:唯一卡号子串(已锚定卡号)。
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents?q="+anchoredID)
	data = body["data"].(map[string]any)
	...
	// ⑤ 详情:card_id 全字段。
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents/"+anchoredID)
	if code := int(body["code"].(float64)); code != 0 {
	...
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/room-cityapi/city/residents/NOPE")
	if code := int(body["code"].(float64)); code != 35042 {
		t.Fatalf("unknown card code = %v, want 35042 (body=%v)", body["code"], body)
	}

	// ⑦ 房间不存在 → 35001。
	_, body = getCityJSON(t, router, "/api/games/virtual_city/rooms/no-such-room/city/residents")
	...
	m2 := virtual_city.NewManager(virtual_city.Config{MonthMs: 3000}, nil)
	m2.SetLoader(profession.NewLoader(cityAPIFixtureDir(t, 4)))
	m2.CreateRoom("room-nocity")
	_, body = getCityJSON(t, cityAPIRouter(m2), "/api/games/virtual_city/rooms/room-nocity/city/residents")
	if code := int(body["code"].(float64)); code != 0 {
