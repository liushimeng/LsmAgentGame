// Package vctypes — 虚拟城市玩家 Agent 的 GameContext 契约类型
// (2026-09-14 §财商流P0)。
//
// 本包与 agent/thptypes/ 同源设计,是 **leaf 包**:不 import game/virtual_city、
// agent/vcplayer、llm/ws(避免循环 import);唯一例外是 import
// agentroot(LsmAgentGame/agent,自身零依赖)以引用 AgentClassCityHuman
// 常量(2026-09-22 §CityHuman重构: 五类 AgentClass 合一,§130 防散写字面量)。
//
// 生命周期约定(与 thptypes 一致):引擎侧(game/virtual_city/agent_runner.go)在
// **持锁态**构造 GameContext 快照;Agent 侧(vcplayer)锁外只读消费。
// 各 Brief 结构的字段名与协议文档 §3 的 JSON 键一一对应。
//
// 详见 lag_docs/虚拟城市/已实现/03-Agent设计/虚拟城市-VCPlayer-Agent设计-v1.md §3。
package vctypes
...
type GameContext struct {
	// 身份元数据
	RoomID   string
	GameKind string // 恒 "virtual_city"
	MySeat   int    // 0..7
	MyUserID string
	ModelKey string
	...
	// 本包不 import game/virtual_city,文案在 game/virtual_city/agent_runner.go 生成)。
...
// 由引擎侧从 profession.Card 映射 —— vcplayer 不 import game/virtual_city,
