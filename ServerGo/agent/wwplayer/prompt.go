...
//
// §20260830-01 §6.2:revealRoleOnDeath 是本局「死亡亮身份」开关(整局不变,
// 由 Agent.SetRevealRoleOnDeath 在首次 wake 前同步)。true 时 §135 身份公开
// 规则段/白痴夜间死亡规则切换为「死亡即公开身份」文案;false 时输出与旧版
// **逐字节一致**(全部既有测试不受影响)。
func BuildSystemPrompt(selfPortrait string, personality PersonalityVector, personalityPresetKey string, difficultyDirective string, revealRoleOnDeath bool) []llm.SystemBlock {
	// §20260830-01 §6.2 — 死亡身份公开规则段双模式(设计文档 §6.2 两套文案)。
	deathRevealRule := "⚠️ 死者身份牌全程不翻开 —— 普通死亡不公开身份,仅「白痴翻牌 / 狼人自爆 / 猎人开枪 / 终局」4 类事件公开。\n"
	if revealRoleOnDeath {
		deathRevealRule = "ℹ️ 本局开启【死亡亮身份】—— 任何玩家死亡/处决时,法官当场公布其身份牌;已公开身份的死者见 user prompt【死亡白名单】(带角色名),引用它不算猜测。\n"
	}
	rules := "【13 人标准竞技局规则】\n" +
	...
	// §20260830-01 §6.2(E 一致性清理):白痴「夜间死亡不翻牌身份保密」仅关闭模式成立;
	// 开启模式下夜间死亡同样当场公开身份,硬编码会与新模式矛盾。
	idiotNightDeathRule := "  夜间被狼刀 / 被毒杀时正常死亡,**不翻牌**,身份保密。\n"
	if revealRoleOnDeath {
		idiotNightDeathRule = "  夜间被狼刀 / 被毒杀时正常死亡,身份按本局【死亡亮身份】规则当场公开。\n"
	}
	...
	// §20260830-01 §6.2 — §135 身份公开规则段双模式(设计文档 §6.2 两套文案)。
	// 关闭 = 现行文案(逐字节一致,零回归);开启 = 死亡即公开身份,公开身份
	// 以【最近死亡事件】/【死亡白名单】块为准,存活玩家身份仍然保密。
	deathIdentityRule := "• 【§135 身份公开规则 — 死者身份牌不翻开】普通死亡出局,**没有人**会自动知道死者身份。\n" +
	...
	if revealRoleOnDeath {
		deathIdentityRule = "• 【§135 身份公开规则 — 本局开启死亡亮身份】任何玩家死亡/处决时,法官当场公布其身份牌。\n" +
			"   - 已公开身份的死亡玩家见 user prompt【死亡白名单】(带角色名),引用它不算猜测,属于合法公开信息;\n" +
			"   - 未死亡玩家的身份仍然保密,严禁声称知道任何存活玩家的具体身份\n" +
			"    (预言家查验结果除外,且只能引用自己的查验)。\n" +
			"   (猎人主动不开枪不亮身份;存活白痴翻牌公开的只是「他是白痴」。)\n"
	}
	// §20260830-01 §6.2 注记:开启模式下追加「死者已公开角色名属于合法公开信息」,
	// 防止 FactCheck 提示词(死亡信息只能引用白名单)与实际合法信息打架;
	// 关闭模式保持空串(输出与旧版逐字节一致)。
	deathClaimLegalNote := ""
	if revealRoleOnDeath {
		deathClaimLegalNote = "   - 死者已公开的角色名属于合法公开信息,可直接引用(本局开启死亡亮身份,以【死亡白名单】为准);\n"
	}
	...
	hardBans := "【系统硬约束 - 极端安全/实现泄漏】\n" +
		...
		// BUG-R10-NEW3-001 (2026-08-16): 白天狼队击杀手势用语泄漏。R10 23:27
		// 报告实测 Bot #1 (小米 mimo-v2.5) 在警长竞选阶段公开发言"我是1号。建议
		// 刀8号，他发言积极，可能是神职。", Bot #2 (美团 LongCat-2.0) 发言"我
		// 觉得刀1号比较合适，你们觉得呢?"。白天公屏出现"我建议/我觉得/我打算/
		// 我应该 + 刀/杀/干掉/带走/投死 + X号"这类击杀手势用语,几乎必然是狼队
		// 内部讨论的脑直陈,与"好人白天不可能用刀/杀谈击杀"的事实违背(R92-P1-1
		// 同类 事实约束)。该用 server 端 ScrubIdentityLeak 加一条 regex 优先抓
		// 这类句式,prompt 层再加一句"白天改用投/怀疑"软引导,双方防御纵深。
		"• 【§R10-NEW3 白天击杀用语禁令】白天(警长竞选/公开发言/投票阶段)公屏\n" +
		"  **严禁**出现「我/咱们 + 建议/觉得/想/打算/准备/决定/应该/要 + 刀/杀/\n" +
		...
		// BUG-R13-NEW-001 (2026-08-17): 神职查验计划泄漏公屏。R13 22:30
		// 自动化测试报告 §二.P0-1 实测 Bot 4 号 (MiniMax M3) 在 D1 警长竞选
		// 阶段公开发言「今晚查 11 号。理由:发言模板化强、像悍跳预言家。…」
		// —— 该发言同时违反两条 §119 协议层隔离原则:
		//   ① 真预言家首夜验完人,公屏应直接报「昨夜我验了 X 是 Y」,
		//     不可能用未来时「今晚要查」;
		//   ② 暴露查验计划 = 让狼队提前知道明晚查验目标 = 直接送出神职
		//     第二条命。R13 实测多 Bot 已能识别(13 号 / 3 号 / 6 号 / 7 号 /
		//     1 号),但人类观感是「先漏后改口」,伤害真实预言家信誉。
		// 根因:LLM system prompt 缺时态约束,与 §R10-NEW3 / §R11-NEW3-001
		// 共同暴露「未来时计划」类句式,需 prompt 层硬约束 + 服务端 hard-reject
		// 双重防御纵深。
		"• 【§R13-NEW 神职时态约束 — 严禁未来时查验/用药/守护计划】\n" +
		"  公屏发言**严禁**使用「今晚/明晚/夜里 + 查/验/守/毒/解/护 + X 号」\n" +
		"  的未来时计划句式(典型违规:「今晚查 11 号」「明晚守 7 号」「今晚我\n" +
		"  解 X 号」「准备验 X 号」)。神职夜间行动在工具调用时已落地,你看到的\n" +
		"  user prompt 任何 private 块已是「昨夜/已」完成态,公屏只能描述过去:\n" +
		"    - 预言家:公屏报查验必须以「昨夜我查验了 X 号,结果是 X 色」开头\n" +
		"      (先报结果,再给推理),**绝不**用「今晚要查 X」/「准备验 X」。\n" +
		"    - 女巫:公屏描述用药必须以「昨夜我用了(解药/毒药)」开头,**绝不**\n" +
		"      用「今晚我会解 X」/「准备毒 X」。\n" +
		"    - 守卫:公屏**不主动**描述守护目标(盲守语义隔离);若被质问,仅\n" +
		"      以「昨夜我守了 X」过去时回应,绝不用「今晚我守 X」未来时。\n" +
		"  违规检测:服务端已新增「future_tense_skill_plan」hard-reject(同 §R11\n" +
		"  范式),命中的发言整条不广播,LLM 收到 reject hint 引导改写为「昨夜 /\n" +
		"  已」过去时表述。\n" +
...
func BuildUserPrompt(ctx wwtypes.GameContext) string {
	...
	if len(ctx.LastNightDeaths) > 0 {
		...
		for _, d := range ctx.LastNightDeaths {
			s += itoa(d+1) + "号"
			// §20260830-01 §6.3 — 死亡亮身份:该座位身份已对全场公开时附带
			// 角色名(数据源 ctx.RevealedRoles,引擎侧 RolePubliclyRevealed 单点
			// 判定派生;非空才显示)。关闭模式普通死亡不在 map → 输出与现状
			// 逐字节一致;开启模式下为空(理论上不应发生)时防御性不写身份。
			if role, ok := ctx.RevealedRoles[d]; ok && role != "" {
				s += "(身份:" + role + ",已公开)"
			}
			s += " "
		}
