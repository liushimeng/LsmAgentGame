// Package vcplayer — prompt.go: System/User prompt 模板(2026-09-14 §财商流P0)。
//
// 契约: Agent 设计文档 §4(System 5 段全文)/ §5(User 模板全文)。
// System 以 SystemBlock 数组产出(每段一个 {"type":"text"},§14.1 wire 约束)。
// 本包不 import game/virtual_city —— 职业卡经 vctypes.CardBrief 投影传入(§2 依赖反转)。
package vcplayer
...
func SystemPromptBlocks(card vctypes.CardBrief) []llmtypes.SystemBlock {
	seg1 := fmt.Sprintf(
...
// insuranceKindCN 险种中文名(P1-4;本包不 import game/virtual_city,与引擎侧
...
func insurancePolicyLine(policies []vctypes.PolicyBrief) string {
	if len(policies) == 0 {
		return ""
	}
...
// game/virtual_city,(此处不 import 引擎侧),故与引擎侧 goods.go 各持一份文案)。
