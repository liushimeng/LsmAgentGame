...
// in-memory VirtualCityRoom 从未登记 bot 座位 —— Start 发卡跳过 bot、
...
// registerVirtualCityAgentSeats 把建房请求中的 agent_seats 注册进 wealth 内存房间。
// 幂等: RegisterBotSeats 只写空座位,重复调用(建房 + 重试)安全。
func (s *GameService) registerVirtualCityAgentSeats(roomID string, seats []service.AgentSeatConfig) *errcode.Error {
	if s.vcMgr == nil {
		return nil
	}
	...
	for _, seatCfg := range seats {
		if seatCfg.Seat < 0 || seatCfg.Seat >= virtual_city.MaxSeats {
			logger.L().Warn("registerVirtualCityAgentSeats: seat out of range",
		...
		if err != nil {
			logger.L().Warn("registerVirtualCityAgentSeats: resolve bot user failed",
