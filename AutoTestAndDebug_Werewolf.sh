#!/usr/bin/env bash
# AutoTestAndDebug_Werewolf.sh
# ---------------------------------------------------------------
# 用途：
#   狼人杀 13 人局专用自动化测试入口。随机选择一个可用的编程 Agent CLI
#   （Claude Code / OpenCode / Codex），读取当前目录或仓库根的
#   AutoTestAndDebug_Werewolf.md 作为提示词执行自动化测试；Agent
#   退出后自动将 TestReport 中以「狼人杀自动化测试报告_」开头的狼人杀
#   报告文件以中文 git 提交（兼容旧 `自动化测试报告_*.md`），子模块
#   UseReport 在子仓库内单独提交。提示词已内嵌「自动修复流程」章节，
#   Agent 测试完成后在同一会话内直接执行修复 + 提交推送，无需外部脚本接力。
#
# 特性：
#   - 工作目录与 Agent 启动目录均为 /usr/local/LsmAgentGame/LsmAgentGame
#   - 支持全部编程 Agent CLI（Claude Code / OpenCode / Codex）
#     随机选择执行；选择逻辑在公共库 agent_cli_common.sh 中
#     （可 source 复用）；AGENT_CLI 环境变量可强制指定某个 Agent
#   - 通过 nohup + setsid + & + disown 脱离调用者，**不阻塞**调用者进程
#   - 日志体系：
#     * 单次运行日志 ./logs/auto_test_debug_werewolf_<Agent程序名>_<timestamp>.log
#     * 运行索引日志 ./logs/auto_run_index.log
#     * 旧运行日志超过保留期（缺省 30 天，LOG_RETAIN_DAYS 可覆盖）自动清理
#   - AutoTestAndDebug_Werewolf.md 优先取当前目录，其次仓库根；都不存在则立即报错退出
...
SCRIPT_TAG="AutoTestAndDebug_Werewolf"
PROMPT_FILE_NAME="AutoTestAndDebug_Werewolf.md"
...
# ---------- 日志文件名含 Agent 程序名 ----------
LOG_FILE="${LOG_DIR}/auto_test_debug_werewolf_${SELECTED_AGENT}_${TS}.log"
...
#   2. 提示词内嵌「自动修复流程」，Agent 测试完成后直接修复 + 提交推送
#   3. Agent 退出后自动 git add / git commit（中文提交信息）
BG_PID="$(start_agent_in_background "${LOG_FILE}" "
    ...
    # 注：TestReport/* 已整目录入 .gitignore(报告处理完即删,不在仓库堆积),
