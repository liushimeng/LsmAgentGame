...
|---|---|---|
| `agent` | `LsmWebGame/agent` | 仅保留 `doc.go` 占位说明 |
| `agentcore` | `LsmWebGame/agent/core` | 通用基础设施 |
| `wwtypes` | `LsmWebGame/agent/wwtypes` | 狼人杀契约（避免与 `LsmWebGame/game/werewolf` 同名混淆） |
| `wwplayer` | `LsmWebGame/agent/wwplayer` | 玩家 Bot Agent |
| `wwjudge` | `LsmWebGame/agent/wwjudge` | 法官 Agent |
...
> **验证方式**：每步迁移完成后 `cd ServerGo && go build -o LsmWebGame main.go && go test ./...`；再 `go list -deps ./... | grep agent` 验证依赖层级单调。
...
| 文件 | 当前 `import "LsmWebGame/agent"` 后调用 | 改动后 |
|---|---|---|
| `ServerGo/main.go` | `agent.SetSummaryBridge` | 新增 `import "LsmWebGame/agent/wwjudge"`；改 `agent.SetSummaryBridge` → `wwjudge.SetSummaryBridge` |
| `ServerGo/ws/chat_service.go` | `agent.RecordLogService`、`agent.BotChatSendResult` | 新增 `import "LsmWebGame/agent/core"` + `"LsmWebGame/agent/wwplayer"`；`agent.RecordLogService` → `agentcore.RecordLogService`、`agent.BotChatSendResult` → `wwplayer.BotChatSendResult` |
| `ServerGo/ws/game_service_werewolf.go` | `agent.GameContext`（1 处） | 新增 `import "LsmWebGame/agent/wwtypes"`；改 `agent.GameContext` → `wwtypes.GameContext` |
...
**所有 `*_test.go` 文件同样需要 import 更新**（grep "LsmWebGame/agent" → 替换为 3 个新包；同一文件可能 import 多个，按需逐个添加）。
...
> 每步必须 `cd ServerGo && go build -o LsmWebGame main.go && go test ./...` 通过后方可提交（§4 硬约束）。
...
8. **批量替换 import**：原 `ServerGo/agent/*.go` 文件内 `agent.ChatHistoryQueue` 等改为 `agentcore.ChatHistoryQueue`；原 import `"LsmWebGame/agent"` 后追加 `"LsmWebGame/agent/core"`；同时 `run_helpers.go` 中仍需要的"游戏相关"helper 暂留原处待 Step 3 处理。
9. `ServerGo/ws/chat_service.go` 与 `ServerGo/config/config.go` 的 import 更新。

**验证**：
```bash
cd /usr/local/LsmWebGame/ServerGo && go build -o LsmWebGame main.go && go test ./agent/core/... ./ws/...
```
...
**验证**：
```bash
cd /usr/local/LsmWebGame/ServerGo && go build -o LsmWebGame main.go && go test ./... -run "^Test" -short
```
...
3. 把所有 `_test.go` 跟随移动。
4. 原 `agent` 包内引用这些文件的符号全部改 import（从 `LsmWebGame/agent` 改为 `LsmWebGame/agent/wwplayer`）。
...
**验证**：
```bash
cd /usr/local/LsmWebGame/ServerGo && go build -o LsmWebGame main.go && go test ./agent/wwplayer/... ./game/werewolf/...
```
...
**验证**：
```bash
cd /usr/local/LsmWebGame/ServerGo && go build -o LsmWebGame main.go && go test ./agent/wwjudge/... ./game/werewolf/...
```
...
3. 验证 `git grep -l "\"LsmWebGame/agent\"" ServerGo/` 应**无结果**（仅 doc.go 提及）。

**验证**：
```bash
cd /usr/local/LsmWebGame/ServerGo && go build -o LsmWebGame main.go && go test ./...
git grep -l '"LsmWebGame/agent"' ServerGo/  # 必须仅剩 doc.go
```
...
```bash
# 1) 编译
cd /usr/local/LsmWebGame/ServerGo && go build -o LsmWebGame main.go

# 2) 全量测试（必须 0 失败）
cd /usr/local/LsmWebGame/ServerGo && go test ./...

# 3) 竞态检测（player + judge 包涉及 goroutine 强路径）
cd /usr/local/LsmWebGame/ServerGo && go test -race ./agent/wwplayer/... ./agent/wwjudge/... ./game/werewolf/...

# 4) 依赖层级审计（验证无反向 import）
cd /usr/local/LsmWebGame/ServerGo && \
...
| R12 | **`config.Load()` 在测试子目录 panic**（§197 测试经验）| 拆分后 wwplayer 测试可能找不到 `LsmWebGame.conf.example` | 测试内 `defer recover()` 兜底 | 与 §197 保持一致 |
