...
| 20260814-02 | **OpenCode 启发的 6 项升级(§20260814-02)**: 分析 OpenCode(`docs/其他Agent代码分析/OpenCode_*.md` 三份)对狼人杀 Agent 提出 6 项改造。**(U1) Provider CachePolicy 4-breakpoint 显式调度**: Anthropic 5m-cache 现有 write=1.25x / read=0.1x 计费,但 cache hit 60-70% 偏低;OpenCode `cache-policy.ts:99-111` "auto"3 断点 + `ANTHROPIC_BREAKPOINT_CAP=4` 启发,补 tools 末尾 + last user + 拆分静态/动态 system 两段。**(U2) WolfWhisper 工具层权限派生**: OpenCode `subagent-permissions.ts:deriveSubagentSessionPermission` 默认 deny 启发,补 WolfPackCipher/Hypothesis/ReasoningChain 三个狼族私有工具的 phase × role 派生校验;`CipherTemplate`/`HypothesisLock`/`ReasoningChainLock` 加服务端兜底。**(U3) LLMSlot 透明背压**: OpenCode `route/executor.ts:rateLimitDetails` 解析 `anthropic-ratelimit-*-{limit,remaining,reset}` headers 启发,新增 `LLMSlot.IsDegraded()` 替代 `endpoint_breaker` 单层熔断;上游 429/529 自适应缩短 Wait 上限。**(U4) BotTranscript 6 类语义分桶**: OpenCode `ToolsCache.shapeExtra` 哈希模式启发,把 60+ JSON 字段拆 6 桶(**A.身份静态** / **B.决策可观测** / **C.情绪** / **D.行为可观测** / **E.死亡公开** / **F.统计元数据**);`recordTranscript` 改为分桶原子 setter,根治 §127 同 turn 多工具调用的整体替换覆盖 bug。**(U5) AgentRunTrace 跨调用追踪 ID + 流式 marker**: OpenCode `assistantMessage.id` + `package.json trace` 启发,补 `AgentRunID` UUID + X-Agent-Run-ID header + 流式 `(traceId, seq, blockKind)` 推送 marker;审计员 `/api/admin/llm/agents/:seat/run/:traceID` 复盘 24h 流。**(U6 ★) AgentWiringLint AST 全字段扫描**: OpenCode 缺 AST 级 lint(Python ABC 强约束) — 用 `go/parser + go/ast` 对 `agent/wwplayer/agent.go::Agent struct` 全字段 grep setter/构造赋值,凡零接线即 CI fail;根治 §130 第八次复发模式。详见 [`docs/狼人杀-重构方案/狼人杀Agent_根据_OpenCode_优化和解决方案_20260814.md`](docs/狼人杀-重构方案/狼人杀Agent_根据_OpenCode_优化和解决方案_20260814.md)。**教训**:(1) **手工 grep 永远漏得快** — §20260812-04 U6 + §20260813-04 U5 的手工 lint 在 §130 第八次复发时**未**咬出 SteeringQueue/ToolHooks setter,改 Go AST 全字段扫描才能根治;(2) **Provider rate-limit headers 必须透明反馈到本地 signal 量** — 否则本地 signal 永远不知道 provider 已满载,会无限重试打爆配额;(3) **transcript 字段严禁整体替换** — §127 第 N 次复现,按 6 类语义分桶才能保证同 turn 多工具调用不互覆盖;(4) **OpenCode plan/build 同 session 切换不适合狼人杀语义** — 没有先 plan 后 build,但 wolf whisper 协议层隔离 + cipher 工具权限派生是关键借鉴 | opencode-inspired-upgrades |
| 20260813-05 | **DeepSeek Harness 借鉴 U2/U3/U5 落地(§20260813-05)**: 分析 dsh `dsh v0.1.0-rc.5`(`docs/其他Agent代码分析/deepseek-harness_*.md` 三份:记忆管理 972 行 / Context 管理 1633 行 / 意图识别与任务分解 2117 行)对狼人杀 Agent 提出 7 项升级,本次落地最高 ROI 的 3 项(U1/U4/U6/U7 留待后续 commit)。**(U2 ★) Runtime Invariant Companion** — 借鉴 dsh `invariant.ts` 模式(`AGENTS.md:103` "owned relationships"):新增 `ServerGo/agent/wwtypes/invariant.go` 12 条不变量(I1–I6 GameContext 字段契约 + I7–I9 消息配对保护 + I10–I12 请求重建一致性),`InvariantViolation{Code,Message,Kind}` + 原子计数器;接线 3 处(`buildAgentContextLocked` 末尾 / `runLoop` 发请求前 / 成功路径后)+ `invariant_wiring_test.go` 跨包 lint 强制 3 个 Check* 都有生产调用点。与 §20260814-02 U6 lint 互补:lint 抓静态字段接线,runtime invariant 抓运行时数据契约。**(U3) Pre-step 主动压缩 + 双触发** — 借鉴 dsh `compaction-basic/src/index.ts:147-223`:新增 `ServerGo/agent/wwplayer/preflight_compress.go` 80% 阈值主动压缩(调 `Memory.CompactWithLLM` + `CompressAndPrune`,失败 fallback `PruneByBytesAggressive`)+ overflow 兜底(已 overflow 且 payload>30% budget 二次触发);`Agent.preflightOverflowCount` 字段 + `resetPreflightOverflowCount` 在 `ResetConsecutiveFailures` 之后调(DSH §8.6 不变量:成功 LLM 调用必清零)。与既有 preflight 三梯度(60% toolResultPruner / 100% PruneByBytes / 400% post-error)协同工作。**(U5) Provider Cache 字节稳定路线** — 借鉴 dsh `agent.ts:465-470` request bytes 稳定路线(不显式下 cache 断点,依赖 provider 自动 server-side KV cache):新增 `prompt_freeze.go::BuildSystemPromptBytes` 一次性冻结 system prompt 字节(5 项测试含 sha256 校验)+ `Agent.systemPromptBytes` 字段(白名单豁免);`invariant I11` 在发请求前比对 `req.System` 字节与 `a.systemPromptBytes`,任何漂移 Debug 日志 + 计数器。**教训**:(1) **runtime invariant 与 CI 静态 lint 是不同形态护栏,互补不可互替** —— lint 抓"声明了却从不接线"(字段存在但无 setter),invariant 抓"运行时数据不变量"(字段存在且 setter 但填错值),二者合并才是根治 §130 第八次复发;(2) **双触发不可移动** —— 单触发(仅 post-error 兜底)让 §197 慢模型场景下数分钟白等;补 80% 主动 + overflow 兜底 + 30% 重试下限 = 廉价与贵重手段协同;(3) **半生不死字段比完全未接线更难察觉** —— §14.1 `CacheControl` 字段整整一个版本反复被文档声明"可挂"却仅 1 处 system 末尾赋值(tools 块无字段),U5 改为"bytes 稳定 + provider 自动 cache"路线从源头消除这类缺陷的伪装;(4) **DSH 哲学一致性比单条 API 借鉴更值钱** —— 日志为真相 + invariant companion 持续守护 + provider 抽象 + 流协议不变量,这四条哲学合并实施比单独借鉴任何一条都更能根治 §130 反复复发。详见 [`docs/狼人杀-重构方案/狼人杀Agent_根据_deepseek-harness_优化和解决方案_20260813.md`](docs/狼人杀-重构方案/狼人杀Agent_根据_deepseek-harness_优化和解决方案_20260813.md) | dsh-inspired-upgrades |
